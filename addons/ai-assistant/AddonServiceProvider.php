...
class AddonServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        Inertia::share('aiAssistant', function () {
            ...
            return [
                ...
                'greeting_on_first_visit' => (bool) addon_setting('ai-assistant', 'greeting_on_first_visit', false),
     ...
     *
     * media_url() applies exactly the policy this method used to hand-roll: ROOT-RELATIVE
     * ("/storage/…") on a local disk rather than the absolute URL Storage::url() builds —
     * that method derives its host from APP_URL, so an install whose APP_URL has drifted
     * from the domain it is actually served on emits a mixed-content image URL the browser
     * blocks. A root-relative path inherits the page's scheme and host, so it cannot
     * mismatch. A cloud disk still gets its real absolute bucket/CDN URL, and a value
     * already stored as a full URL is left untouched.
     */
    private function avatarUrl(?string $value): ?string
    {
        return media_url($value) ?: null;
    }
