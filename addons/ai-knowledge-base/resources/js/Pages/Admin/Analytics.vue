...
<template>
    <AdminLayout :title="t('KB Analytics')">
        <Head :title="t('KB Analytics')" />
        <div class="w-full px-4 sm:px-6 lg:px-6 xl:px-8 2xl:px-10">
            <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div class="flex flex-wrap items-center gap-3">
                        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('Knowledge Base Analytics') }}</h1>
            ...
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <!--
                    Embedding progress sits in the fourth slot rather than in a panel of its
                    own further down. It is a set of counts with no period to compare against,
                    so it is laid out as status chips inside a StatsCard-shaped shell instead
                    of borrowing the value/comparison treatment of the three cards beside it.
                -->
                <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition-all duration-300 hover:border-gray-300 dark:border-surface-800 dark:bg-surface-900 dark:hover:border-surface-700">
                    <div class="mb-3 flex items-start justify-between gap-3">
                        <div>
                            <p class="text-sm font-medium text-gray-400">{{ t('Embed Status') }}</p>
                        </div>
                        <div class="flex h-9 w-9 items-center justify-center rounded-xl border border-accent-500/10 bg-gradient-to-br from-accent-500/10 to-accent-500/5 text-accent-400">
                            <i class="ti ti-vector-triangle text-lg"></i>
                        </div>
                    </div>

                    <div v-if="embedSummaryItems.length">
                        <div  class="flex flex-wrap gap-x-4 gap-y-2">
                            <div v-for="item in embedSummaryItems" :key="item.key" class="min-w-[64px]">
                                <p class="text-lg font-bold leading-tight text-gray-900 dark:text-white">
                                    {{ item.count }}
                                </p>
                                <p class="text-[11px] font-semibold uppercase tracking-[0.06em] text-gray-400">
                                    {{ item.label }}
                                </p>
                            </div>
                        </div>
                        <p class="mt-0.5 text-xs text-gray-400 dark:text-gray-500">
                            {{ t('Article embedding progress') }}
                        </p>
                    </div>
            ...
            <section class="mt-6 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-surface-800 dark:bg-surface-900">
