...
/**
 * An asset records the model it was made with as a raw provider slug
 * (`gemini-3.1-flash-image-preview`), which is an implementation detail, not a label.
 * Resolve it against the catalogue the admin configured to get the display name and
 * provider they chose.
 *
 * Falls back to the slug when the model is not in the catalogue — an admin can remove a
 * model after images were made with it, and a stale slug still beats an empty field.
 */
export function modelLabel(
    ...
    const known = models.find((model) => model.slug === slug)

    return known
        ? { name: known.name, provider: known.provider }
        : { name: slug, provider: null }
}
