...
import { useGsapScrollAnimation } from '@themes/default/js/composables/useGsapScrollAnimation'
...
interface Seo {
    /** Complete document title (with site name) — consumed by app.blade.php, not here. */
    title: string
    /** Site-free base for <Head>; the global callback appends the site name. */
    title_page: string
    description: string
}
...
/* ------------------------------------------------------------------ *
 * Scroll reveal — the same GSAP idiom as the homepage Sections.
 *
 * The hero plays an intro timeline on load; every section below fades
 * and lifts its heading, then staggers its items in as it scrolls into
 * view (`start: 'top 85%'`, `once: true`). `useGsapScrollAnimation` owns
 * the context so it is reverted on unmount — no manual onUnmounted here.
 *
 * All of it is gated behind prefers-reduced-motion and none of it sets an
 * initial hidden state in CSS, so a reduced-motion visitor (or a failed
 * gsap import) still sees the page fully painted.
 * ------------------------------------------------------------------ */
const rootRef = ref<HTMLElement | null>(null)
const { setContext } = useGsapScrollAnimation()
...
onMounted(async () => {
    ...
    const ctx = gsap.context(() => {
        // Hero — intro timeline, plays immediately (it's above the fold).
        const tl = gsap.timeline({ defaults: { ease: 'power3.out' } })
        const badge = root.querySelector('.aip-hero-badge')
        const heading = root.querySelector('.aip-hero-heading')
        const sub = root.querySelector('.aip-hero-sub')
        const panel = root.querySelector('.aip-hero-panel')
        if (badge) tl.from(badge, { y: 20, opacity: 0, duration: 0.6 })
        if (heading) tl.from(heading, { y: 40, opacity: 0, duration: 0.8 }, badge ? '-=0.4' : '+=0')
        if (sub) tl.from(sub, { y: 30, opacity: 0, duration: 0.7 }, '-=0.5')
        if (panel) tl.from(panel, { y: 30, opacity: 0, duration: 0.7 }, '-=0.4')

        // Every section below — heading first, then staggered items, on scroll.
        root.querySelectorAll<HTMLElement>('.aip-section').forEach((section) => {
            const scrollTrigger = { trigger: section, start: 'top 85%', once: true }
            const sectionHeading = section.querySelector('.aip-heading')
            const items = section.querySelectorAll('.aip-item')
            ...
            if (items.length) {
                gsap.from(items, {
                    opacity: 0, y: 50, duration: 0.6, stagger: 0.1, ease: 'power2.out',
                    immediateRender: false, scrollTrigger,
                })
            }
        })

        // Features — a richer, per-row reveal for the premium feel. Each zigzag
        // row animates its own parts: the media card glides in from the side it
        // sits on (left on even rows, right on odd), while the copy's children —
        // heading, body, button — stagger up beside it. The section's own
        // overflow-hidden contains the horizontal glide so it never nudges a
        // scrollbar. (These rows carry no `.aip-item`, so the generic pass above
        // only touches the Features heading, not the rows.)
        root.querySelectorAll<HTMLElement>('.aip-feature-row').forEach((row, index) => {
            const scrollTrigger = { trigger: row, start: 'top 80%', once: true }
            const media = row.querySelector('.aip-feature-media')
            const copy = row.querySelector('.aip-feature-copy')
    ...
    // Sections that mount tall (lazy images, accordion) settle their trigger
    // positions a tick after paint — same refresh the homepage Sections do.
    setTimeout(() => ScrollTrigger.refresh(), 100)
})
</script>

<template>
    <!-- Site-free base only: the global callback (app.ts) appends " <sep> <site>" to a
         <title> child just as it does to the :title prop. seo.title is the COMPLETE tag
         (used by app.blade.php), so passing it here would render "… | MakeAI | MakeAI". -->
    <Head>
        <title>{{ seo.title_page || toolName }}</title>
        <meta v-if="seo.description" name="description" :content="seo.description" />
    </Head>

    <div ref="rootRef" :class="pageClass" class="aip-landing">
        <div :class="shellClass">
                ...
                <div :class="sectionClass" class="relative pb-16 pt-8 sm:pb-20 lg:pt-12">
                    ...
                    <div class="text-center">
                        <p
                            ...
                            class="aip-hero-badge mb-6 inline-flex items-center gap-1.5 rounded-full border border-gray-200/80 bg-white/70 px-3.5 py-1.5 text-xs font-medium text-gray-600 shadow-sm backdrop-blur dark:border-surface-700 dark:bg-surface-900/70 dark:text-gray-300"
                        >
                        ...
                        <h1
                            class="aip-hero-heading mx-auto max-w-4xl text-balance pb-1 text-3xl font-semibold leading-[1.15] tracking-tight sm:text-5xl sm:leading-[1.12]"
                            :class="headingClass"
                        >
                            {{ content.heroHeading }}
                        </h1>
                        ...
                        <p
                            ...
                            class="aip-hero-sub mx-auto mt-5 max-w-2xl text-pretty text-sm leading-relaxed text-gray-500 sm:text-base dark:text-gray-400"
                        >
                    ...
                    <!-- Prompt panel -->
                    <div ref="promptPanel" class="aip-hero-panel mx-auto mt-10 max-w-3xl scroll-mt-24">
                        <LandingPromptPanel
                            :models="models"
             ...
             * ========================================================== -->
            <section v-if="showFeatures" class="aip-section overflow-hidden py-16 sm:py-24">
                <div :class="sectionClass">
                    <div class="aip-heading mx-auto max-w-2xl text-center">
                        <h2
                    ...
                    <div class="mt-14 divide-y divide-gray-100 dark:divide-surface-800">
                        <div
                            ...
                            :key="`feature-${index}`"
                            class="aip-feature-row grid items-center gap-8 py-12 first:pt-0 last:pb-0 lg:grid-cols-2 lg:gap-16 lg:py-16"
                        >
                            ...
                            <div :class="index % 2 === 1 ? 'lg:order-2' : 'lg:order-1'">
                                <div
                                    class="aip-feature-media relative h-44 overflow-hidden rounded-2xl bg-gray-50 shadow-[0_10px_40px_-16px_rgba(15,23,42,0.25)] ring-1 ring-gray-900/5 sm:h-52 lg:h-56 dark:bg-surface-900 dark:ring-white/5"
                                >
                                    <img
                            ...
                            <!-- Copy -->
                            <div class="aip-feature-copy" :class="index % 2 === 1 ? 'lg:order-1' : 'lg:order-2'">
                                <h3
             ...
             * ========================================================== -->
            <section v-if="showUsecases" class="aip-section relative py-16 sm:py-24">
                ...
                <div :class="sectionClass" class="relative">
                    ...
                    <div class="mt-12 grid gap-5 sm:grid-cols-2">
                        <div
                            ...
                            :key="`usecase-${index}`"
                            class="aip-item rounded-2xl border border-gray-200/80 bg-white p-6 transition duration-200 hover:border-gray-300 hover:shadow-[0_10px_30px_-16px_rgba(15,23,42,0.2)] sm:p-7 dark:border-surface-700 dark:bg-surface-900 dark:hover:border-surface-600"
                        >
             ...
             * ========================================================== -->
            <section v-if="showBenefits" class="aip-section border-t border-gray-100/80 py-16 sm:py-24 dark:border-surface-800">
                <div :class="sectionClass">
                    <div class="aip-heading mx-auto max-w-2xl text-center">
                        <h2
                    ...
                    <div class="mt-14 grid gap-10 sm:grid-cols-3 sm:gap-8">
                        <div
                            ...
                            :key="`benefit-${index}`"
                            class="aip-item flex flex-col items-center px-2 text-center"
                        >
                            <span
             ...
             * ========================================================== -->
            <section v-if="showSteps" class="aip-section relative py-16 sm:py-24">
                <div aria-hidden="true" :class="tintBandClass"></div>
                <div :class="sectionClass" class="relative">
                    <div class="aip-heading mx-auto max-w-2xl text-center">
                        <h2
                    ...
                    <ol class="mt-14 grid gap-6 md:grid-cols-3">
                        <li
                            ...
                            :key="`step-${index}`"
                            class="aip-item overflow-hidden rounded-2xl border border-gray-200/80 bg-white shadow-[0_8px_30px_-18px_rgba(15,23,42,0.2)] dark:border-surface-700 dark:bg-surface-900"
                        >
             ...
             * ========================================================== -->
            <section v-if="showAbout" class="aip-section border-t border-gray-100/80 py-16 sm:py-20 dark:border-surface-800">
                <div :class="proseClass" class="aip-heading">
                    <h2
             ...
             * FAQ
             * ========================================================== -->
            <section v-if="showFaqs" class="aip-section border-t border-gray-100/80 py-16 sm:py-24 dark:border-surface-800">
                <div :class="proseClass">
                    <h2
                        v-if="content.faqHeading"
                        class="aip-heading mx-auto mb-12 max-w-xl text-balance pb-1 text-center text-3xl font-semibold leading-tight tracking-tight sm:text-4xl"
                        :class="headingClass"
                    >
                        {{ content.faqHeading }}
                    </h2>

                    <FaqAccordion :items="faqs" id-prefix="aip-faq" class="aip-item" />
                </div>
            </section>
             ...
             * CTA BANNER
             * ========================================================== -->
            <section v-if="showCta" class="aip-section pb-20 pt-4 sm:pb-28">
                <div :class="sectionClass">
                    <div
                        class="aip-heading relative overflow-hidden rounded-3xl bg-gray-950 px-6 py-20 text-center shadow-2xl sm:px-12 sm:py-24"
                    >
