...
class ImageProSeeder extends Seeder
{
    ...
    {
        // Check persistence against the addon_settings store the value is actually written
        // to (AddonSetting, keyed by slug + bare key). The old check used
        // Setting::isPersisted('addon_{slug}_{key}') against the core settings table, which
        // no longer holds addon rows since the Phase 2 refactor moved them out — so it
        // always returned false and re-seeded (clobbering operator edits) on every activation.
        if (AddonSetting::isPersisted(OperationRegistry::SLUG, $key)) {
            return;
        }
