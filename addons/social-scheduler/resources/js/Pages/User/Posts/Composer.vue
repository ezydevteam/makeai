...
function applyBestTime() {
    if (bestTimeResult.value) {
        form.scheduled_at = toDateTimeLocalInput(bestTimeResult.value.suggested_time)
        bestTimeResult.value = null
        bestTimeError.value = ''
    }
}
...
function submit() {
    // The picker holds the user's local wall clock; the queue fires on an instant. The
    // prefill above already converts UTC -> local, and this is the return leg: without
    // it a post scheduled for 9am publishes at 9am UTC instead.
    form.transform((data) => ({
        ...data,
