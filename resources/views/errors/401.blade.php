@php
    ...
    $primaryActionLabel = $isAdminError ? translate('Admin Sign In') : translate('Sign In');
    $secondaryActionUrl = $isAdminError ? url('/') : url('/register');
    $secondaryActionLabel = $isAdminError ? translate('Go to Homepage') : translate('Create Account');
@endphp
<!doctype html>
...
<head>
    ...
    <style>
        :root {
            --p: {{ $primaryColor }};
            --bg: #eff6ff; --card: #fff;
            --border: #bfdbfe; --t1: #1e1b4b; --t2: #4b5563;
            --radius: 16px;
        }
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: linear-gradient(135deg, #eff6ff, #f0f5ff); color: var(--t2); font-family: Inter, system-ui, sans-serif; }
        .page-shell {
...
<main class="panel">
    ...
    <div class="code" aria-label="401">
        ...
        <span class="code-digit code-digit-last">1</span>
    </div>
    <h1>{{ translate('Sign in required') }}</h1>
    <p>{{ translate('You need to sign in to access this page. Create a free account or log in to continue.') }}</p>
    <div class="actions">
