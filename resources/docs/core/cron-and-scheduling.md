...
**Admin → System → Cron Jobs** shows a Configured/Setup Required badge based on whether the
scheduler has run within the last five minutes, plus the exact cron line to copy, your
project path, and PHP binary. Below that is a table of every scheduled task — daily allowance
resets, subscription renewal reminders, notification cleanup, and more — each with its next
run time, its last manual run, and a **Run Now** button you can use to fire it immediately
without waiting for its schedule. Run Now is disabled until the scheduler itself is detected
as configured.

The AI Assistant's `/health` command reports the same cron status directly, and is the
quickest check if you'd rather not open the System screen.
