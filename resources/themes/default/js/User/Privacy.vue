...
const props = defineProps<{
    ...
    optinPreferencesEnabled: boolean
    cookiePreferencesEnabled: boolean
}>()
...
const preferencesForm = useForm({
    ...
    sms_marketing_opt_in: props.user?.sms_marketing_opt_in ?? true,
    cookie_consent: {
...
// Self-service account deletion toggle (Admin → Settings → Features). Hides the
// "Delete My Account" action when off; the server also 404s the schedule route.
// The cancel banner stays available so a pending deletion can still be cancelled.
const accountDeletionEnabled = computed(() => page.props.accountDeletionEnabled !== false)
...
<template>
    ...
    <div class="space-y-6">
        ...
        <!-- Preferences -->
        <section v-if="optinPreferencesEnabled" class="rounded-2xl border border-gray-200 bg-white p-6 shadow-[0_18px_40px_rgba(15,23,42,0.06)] dark:border-surface-800 dark:bg-surface-900">
            ...
            <div class="mt-4 space-y-5">
                ...
                <!-- Only offered to users we can actually text; a verified phone proves
                     ownership, this checkbox is the separate consent to be messaged. -->
                <label v-if="smsAvailable" class="flex items-center gap-3">
                    <input v-model="preferencesForm.sms_marketing_opt_in" type="checkbox" class="w-4 h-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
                    <div>
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('SMS Updates') }}</span>
                        <p class="text-xs text-gray-500">{{ t('Receive product updates and announcements by text message. Security codes are always sent regardless.') }}</p>
                    </div>
                </label>
        ...
        <!-- Cookie Preferences -->
        <section v-if="cookiePreferencesEnabled" class="rounded-2xl border border-gray-200 bg-white p-6 shadow-[0_18px_40px_rgba(15,23,42,0.06)] dark:border-surface-800 dark:bg-surface-900">
         ...
         <!-- Account Deletion -->
        <section v-if="accountDeletionEnabled" class="rounded-2xl border border-gray-200 bg-white p-6 shadow-[0_18px_40px_rgba(15,23,42,0.06)] dark:border-surface-800 dark:bg-surface-900">
