            ...
            <div class="relative z-20 flex flex-col gap-3 border-b border-gray-100 p-4 sm:flex-row sm:items-end sm:justify-between dark:border-surface-800">
                <div class="relative flex-1 min-w-[220px] md:max-w-sm">
                    ...
                    <input
                        v-model="search"
                        type="text"
                        :placeholder="t('Search support tickets...')"
                        class="w-full rounded-xl border border-gray-200 bg-gray-50 py-2 pl-11 pr-10 text-sm text-gray-900 outline-none transition focus:border-primary-500 focus:ring-2 focus:ring-primary-500/15 dark:border-surface-700 dark:bg-surface-800 dark:text-white"
                        @keyup.enter="filter"
                    >
                    <button
                        v-if="search"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-100 dark:divide-surface-800">
                    <thead class="bg-gray-50 dark:bg-surface-800">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500">{{ t('Ticket') }}</th>
                            <th class="px-4 py-3 text-center text-xs font-semibold uppercase text-gray-500">{{ t('Department') }}</th>
                            <th class="px-4 py-3 text-center text-xs font-semibold uppercase text-gray-500">{{ t('Priority') }}</th>
                            <th class="px-4 py-3 text-center text-xs font-semibold uppercase text-gray-500">{{ t('Status') }}</th>
                            <th class="px-4 py-3 text-center text-xs font-semibold uppercase text-gray-500">{{ t('Last Reply') }}</th>
                            <th class="px-4 py-3 text-right text-xs font-semibold uppercase text-gray-500">{{ t('Action') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-surface-800">
                        ...
                        <tr v-if="tickets.data.length === 0">
                            <td colspan="6" class="px-4 py-12 text-center text-sm text-gray-500">
                                <span v-if="search">{{ t('No results found for ":query"', { query: search }) }}</span>
                                <span v-else-if="status !== 'all'">{{ t('No support tickets match this status.') }}</span>
                                <span v-else>{{ t('No support tickets found.') }}</span>
                            </td>
                        </tr>
                    </tbody>
                </table>
