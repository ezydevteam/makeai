...
const displayPrice = (plan: Plan) => {
    ...
    if (plan.pricing?.is_localized && cycle.display_formatted) {
        return cycle.display_formatted
    }
...
type PlanCta = { label: string; kind: 'link' | 'downgrade' | 'upgrade-now' | 'current'; href?: string }
                ...
                <div v-for="plan in plans" :key="plan.id" :class="planCardClass(plan)">
                    ...
                    <div class="mb-6">
                        <div class="flex flex-wrap items-end gap-2">
                            <span v-if="!plan.pricing?.is_localized && activeCycle(plan).original_formatted && Number(activeCycle(plan).original_amount) > activeCycle(plan).subtotal_amount" class="mb-1 text-lg font-bold text-gray-400 dark:text-gray-500 line-through">
