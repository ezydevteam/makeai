...
const method = computed(() => props.twoFactorMethod ?? 'totp')
const isSms = computed(() => method.value === 'sms')
// Both SMS and email deliver a 6-digit one-time code (vs. the authenticator's TOTP).
const isOtp = computed(() => method.value === 'sms' || method.value === 'email')
...
const resend = () => {
    resendForm.post(route('two-factor.resend'), { preserveScroll: true })
}
</script>
                ...
                <div class="auth-card mx-auto rounded-2xl border border-gray-200 bg-white px-6 py-7 shadow-none dark:border-white/10 dark:bg-surface-950 sm:px-7">
                    <div class="auth-brand-block mb-8">
                        ...
                        <p class="mt-2 max-w-sm text-sm leading-6 text-gray-500 dark:text-gray-400">
                            <template v-if="isSms">
                                {{ $t('Enter the 6-digit code we texted to your phone to finish signing in.') }}
                            </template>
                            <template v-else-if="isOtp">
                    ...
                    <form class="space-y-5" @submit.prevent="submit">
                        ...
                        <p v-if="isOtp" class="text-center text-sm text-gray-500 dark:text-gray-400">
                            ...
                            <button type="button" :disabled="resendForm.processing" class="auth-inline-link disabled:opacity-60" @click="resend">
                                {{ resendForm.processing ? $t('Sending...') : $t('Resend code') }}
                            </button>
                        </p>
