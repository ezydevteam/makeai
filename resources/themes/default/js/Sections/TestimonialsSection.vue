...
interface Testimonial { id: number; name: string; role: string | null; company: string | null; avatar: string | null; content: string; rating: number; is_featured: boolean; source: string; show_source?: boolean }
...
// Public-facing source badge, shown only when the admin enabled "Show Source Publicly" on a
// testimonial. Unknown sources render nothing.
const sourceMeta = (source: string): { label: string; icon: string } | null => ({
    google: { label: 'Google', icon: 'ti ti-brand-google' },
    trustpilot: { label: 'Trustpilot', icon: 'ti ti-star-filled' },
    manual: { label: t('Verified'), icon: 'ti ti-rosette-discount-check' },
    ai: { label: t('AI'), icon: 'ti ti-sparkles' },
}[source] ?? null)
const testimonialSource = (testimonial: Testimonial) => testimonial.show_source ? sourceMeta(testimonial.source) : null
                ...
                <div :class="[sectionHeaderClass(asString(section.config.title_align, 'center'))]" class="mb-16">
                    ...
                    <div class="w-full">
                        ...
                        <div :class="['flex items-center gap-3 mb-4', titleAlignClass(asString(section.config.title_align, 'center')) === 'text-center' ? 'justify-center' : 'justify-start']">
                            ...
                            <h2 :class="['font-black', titleSizeClass(asString(section.config.title_size, 'md')), titleColorClass(asString(section.config.title_color, 'dark'))]">{{ t(asString(section.config.heading ?? section.config.title, t('What Our Users Say'))) }}</h2>
                        </div>
                    ...
                    >
                        <SwiperSlide v-for="testimonial in getTestimonialsSlice()" :key="testimonial.id" class="!h-auto flex flex-col">
                            <div :class="[sliderColumns === 1 ? 'relative flex h-full flex-col gap-6 p-4 text-center items-center justify-center bg-transparent border-0 shadow-none' : testimonialsCardClass(asString(section.config.card_style, 'bordered')), 'h-full flex-1 testimonial-card']">
                                ...
                                <div class="relative z-10 flex items-center gap-2" :class="[isCentered ? 'justify-center' : '']">
                                    ...
                                    <span
                                        v-if="testimonialSource(testimonial)"
                                        :title="t('Review from :source', { source: testimonialSource(testimonial)!.label })"
                                        class="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2 py-0.5 text-[11px] font-medium !text-gray-500 dark:bg-surface-800 dark:!text-gray-400"
                                    >
                                        <i :class="testimonialSource(testimonial)!.icon"></i>
                                        {{ testimonialSource(testimonial)!.label }}
                                    </span>
                                </div>
                                ...
                                <div class="relative z-10 flex border-t border-gray-100 pt-4 dark:border-surface-700 w-full" :class="[isCentered ? 'flex-col items-center justify-center border-t-0 gap-2' : 'items-center gap-3']">
                                    ...
                                    <div class="min-w-0" :class="[isCentered ? 'text-center' : '']">
                                        <p class="truncate text-md font-semibold !text-gray-900 dark:!text-white">{{ testimonial.name }}</p>
