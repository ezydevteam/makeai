...
import { TITLE_COLOR_MAP } from '../composables/useSectionStyle'
...
const asBoolean = (value: SectionConfigValue | undefined, fallback = false): boolean => {
...
// Hero-only values. 'light'/'white' are what effectiveHeadingColor swaps in over a dark
// background, so they are not offered in the picker; blue/red are legacy stored values.
const HERO_ONLY_COLORS: Record<string, { heading: string; subheading: string }> = {
    light: { heading: '!text-white', subheading: '!text-white/70' },
    white: { heading: '!text-white', subheading: '!text-white/70' },
    blue: { heading: 'text-blue-600 dark:text-blue-400', subheading: 'text-blue-500/80 dark:text-blue-300/80' },
    red: { heading: 'text-red-600 dark:text-red-400', subheading: 'text-red-500 dark:text-red-300' },
}

// Everything else resolves through the same palette the other sections' Title Color uses,
// so the hero picker can offer the identical option list (including the gradients).
const heroColorClass = (color: string, tone: 'heading' | 'subheading' = 'heading'): string => {
    const heroOnly = HERO_ONLY_COLORS[color]
    if (heroOnly) return tone === 'heading' ? heroOnly.heading : heroOnly.subheading

    const shared = TITLE_COLOR_MAP[color] ?? TITLE_COLOR_MAP.dark
    return tone === 'heading' ? shared.title : shared.subtitle
}
...
// Scaled down below sm: two side-by-side CTAs at full desktop size overflow a phone. The
// sm: values are the original sizes, so nothing changes from 640px up.
const heroButtonSizeClass = (size: string): string => ({
    ...
    lg: 'px-6 py-3.5 text-base sm:px-10 sm:py-5 sm:text-lg',
    xl: 'px-7 py-4 text-base sm:px-12 sm:py-6 sm:text-xl',
}[size] ?? 'px-5 py-3 text-sm sm:px-8 sm:py-4 sm:text-base')
...
// The overlay strength is the only control, so 0 has to mean "no overlay" rather than
// falling through to the default the way `Number(x) || undefined` would.
const overlayOpacity = computed((): number | undefined => {
    const raw = props.section.config.overlay_opacity
    if (raw === undefined || raw === null || raw === '') return undefined
    const n = Number(raw)
    return Number.isFinite(n) ? n : undefined
})

// Admins type "#home" as often as "home"; a leading # would render id="#home" and break the
// very link it exists for, so strip it.
const sectionAnchorId = (anchor: unknown): string | undefined => {
...
const mobileHeroHeaderOffset = computed(() => {
    const mobile = frontendHeaderSettings.value.mobile ?? {}
    const desktop = frontendHeaderSettings.value.desktop ?? {}

    if (String(desktop.transparent_on_hero) !== 'true') return 0

    return Number(mobile.height ?? 64)
})
...
const heroSpacingStyle = (basePadding: number): Record<string, string> => {
    ...
    const topMobile = basePadding + mobileHeroHeaderOffset.value
    return {
        paddingTop: `${top}px`,
        '--hero-padding-top': `${top}px`,
        paddingBottom: `${basePadding}px`,
        '--hero-padding-bottom': `${basePadding}px`,
        '--hero-padding-top-mobile': `${topMobile}px`,
    }
}
...
<template>
    ...
    <section v-if="heroLayoutVariant === 'centered-gradient'"
        data-home-hero="true"
        :id="sectionAnchorId(props.section.config.section_anchor)"
    ...
    >
        <div v-if="asBoolean(props.section.config.show_hero_background, false) && asString(props.section.config.hero_background_url)" class="absolute inset-0 z-0">
            <img v-if="asString(props.section.config.hero_background_type, 'image') === 'image'" :src="resolveMediaUrl(asString(props.section.config.hero_background_url))" alt="" loading="eager" fetchpriority="high" decoding="async" class="h-full w-full object-cover">
        ...
        <div v-if="(hasBackgroundMedia || (gradientEnabled && isBackgroundDark)) && asBoolean(props.section.config.show_hero_gradient_overlay, true)" :style="sectionOverlayStyle(overlayOpacity)" class="absolute inset-0 z-10 bg-gradient-to-b from-black/70 via-black/45 to-black/25"></div>
            ...
            <div class="grid items-center gap-16 lg:grid-cols-2">
                <div>
                    ...
                    <div v-if="asBoolean(props.section.config.show_search_box, true)" class="mb-8 w-full max-w-lg">
                        <button type="button" @click="openCommandPalette()"
                            :class="[
                                'flex w-full items-center gap-3 rounded-full border px-5 py-3 text-left text-sm backdrop-blur-sm shadow transition-all',
                                isDarkGradientSelected
                                    ? 'border-white/20 bg-white/10 text-white/60 hover:bg-white/20 hover:shadow-none shadow-none'
                                    : (isBackgroundDark ? 'border-white/20 bg-white/15 text-white/60 hover:bg-white/25 shadow-lg hover:shadow-xl' : 'border-white/30 bg-white/90 text-gray-400 shadow-lg hover:bg-white hover:shadow-xl dark:border-surface-700 dark:bg-surface-800/90 dark:text-gray-500 dark:hover:bg-surface-800')
                            ]"
                        >
                            <i class="ti ti-search text-base"></i>
                            <span class="flex-1">{{ t('Search tools, pages & more...') }}</span>
                    ...
                    <div class="flex flex-col gap-4 sm:flex-row gsap-cta">
                        <button v-if="asBoolean(props.section.config.show_primary_cta, true) && asString(props.section.config.primary_cta_text) && checkAccessLevel(asString(props.section.config.primary_cta_access_level, 'all'))"
                            @click="handlePrimaryCtaClick"
        ...
        <div class="relative z-20 mx-auto w-full max-w-7xl px-6 py-24">
            <div class="grid items-center gap-16 lg:grid-cols-2">
                ...
                <div v-if="asString(props.section.config.hero_split_image_url)" class="flex items-center justify-center gsap-image">
                    <img :src="resolveMediaUrl(asString(props.section.config.hero_split_image_url))" alt="" loading="eager" fetchpriority="high" decoding="async" class="h-auto w-full max-w-lg rounded-2xl shadow-2xl" />
                </div>
                ...
                <div class="mt-16 w-full max-w-5xl gsap-image">
                    <div class="overflow-hidden rounded-2xl shadow-2xl ring-1 ring-black/5">
                        ...
                        <div v-if="asString(props.section.config.hero_split_image_url)" class="bg-white dark:bg-surface-900">
                            <img :src="resolveMediaUrl(asString(props.section.config.hero_split_image_url))" alt="" loading="eager" fetchpriority="high" decoding="async" class="w-full" />
                        </div>
    ...
    >
        ...
        <div v-if="!gradientEnabled && !hasBackgroundMedia" class="pointer-events-none absolute inset-0 z-0 opacity-[0.04] dark:opacity-[0.06]" :style="enterpriseGridStyle"></div>
        <div v-if="(hasBackgroundMedia || (gradientEnabled && isBackgroundDark)) && asBoolean(props.section.config.show_hero_gradient_overlay, true)" :style="sectionOverlayStyle(overlayOpacity)" class="absolute inset-0 z-10 bg-gradient-to-b from-black/70 via-black/45 to-black/25"></div>
        <div class="relative z-20 mx-auto w-full max-w-6xl px-6">
            <div class="mx-auto w-full max-w-2xl text-center">
                ...
                <div class="mx-auto !max-w-3xl">
                    ...
                    <div v-if="asBoolean(props.section.config.show_search_box, true)" class="mb-8 w-full">
                        <button type="button" @click="openCommandPalette()"
                            :class="[
                                'flex w-full items-center gap-3 rounded-full border px-5 py-3 text-left text-sm backdrop-blur-sm transition-all',
                                hasBackgroundMedia || isDarkGradientSelected
                                    ? 'border-white/20 bg-white/10 text-white/60 hover:bg-white/20 hover:shadow-none shadow-none'
                                    : 'border-white/30 bg-white/85 text-gray-400 shadow-lg hover:bg-white hover:shadow-xl dark:border-surface-700 dark:bg-surface-800/90 dark:text-gray-500 dark:hover:bg-surface-800'
                            ]"
                        >
                ...
                <div class="flex flex-col justify-center gap-4 sm:flex-row gsap-cta">
                    <button v-if="asBoolean(props.section.config.show_primary_cta, true) && asString(props.section.config.primary_cta_text) && checkAccessLevel(asString(props.section.config.primary_cta_access_level, 'all'))"
                        @click="handlePrimaryCtaClick"
            ...
            <div v-if="asBoolean(props.section.config.show_stats, true) && asItems(props.section.config.stats).length > 0"
                :class="asBoolean(props.section.config.show_stats_separator, true) ? ['mt-24 grid grid-cols-2 gap-8 sm:grid-cols-4 border-t pt-12', isBackgroundDark ? 'border-white/10' : 'border-gray-100 dark:border-surface-800'] : 'mt-24 grid grid-cols-2 gap-8 sm:grid-cols-4'">
                <div v-for="stat in asItems(props.section.config.stats)" :key="`${stat.number}_${stat.label}`" class="text-center">
                    <p v-if="gradientEnabled" :class="[heroGradientTextClass, 'text-2xl font-black sm:text-3xl']">{{ stat.number }}</p>
...
/*
 * Hold the scroll while a tool is hovered so it can actually be clicked. Pausing on the
 * whole strip rather than the individual card means the tool under the cursor cannot slide
 * away while it is being aimed at. Guarded by hover:hover so a tap on touch does not leave
 * the marquee stuck paused.
 */
@media (hover: hover) {
    .display-marquee:hover .display-marquee-inner {
        animation-play-state: paused;
    }
}

/* Keyboard users tabbing through the tools get the same hold. */
.display-marquee:focus-within .display-marquee-inner {
    animation-play-state: paused;
}
