...
// Configs saved with an image upload store flags as "1"/"0" strings, so accept both.
const asBool = (v: SectionConfigValue | undefined, fallback = false): boolean => {
    if (v === undefined || v === null || v === '') return fallback
    if (typeof v === 'boolean') return v
    return !['0', 'false', 'no', 'off'].includes(String(v).trim().toLowerCase())
}

// Captions are hidden on the cards only — the lightbox still shows title and description.
const showCardCaptions = computed(() => ! asBool(props.section.config.hide_captions, false))
...
// Slide widths and the track transform are inline styles derived from the column count, so the
// mobile override has to be reactive rather than a CSS breakpoint class.
const isMobile = ref(false)
let mobileQuery: MediaQueryList | null = null
const syncIsMobile = (e: MediaQueryList | MediaQueryListEvent) => { isMobile.value = e.matches }

// Below md the carousel always shows one card at a time — three columns on a phone leaves each
// image a few dozen pixels wide.
const effectiveColumns = computed(() => (isMobile.value ? 1 : Number(columnsCount.value)))
...
const maxIndex = computed(() => Math.max(0, items.value.length - effectiveColumns.value))
...
// Rotating a phone to landscape shrinks maxIndex; without this the track stays scrolled past
// the last slide and the carousel shows blank space.
watch(maxIndex, (max) => {
    ...
    <section
        ...
        <div class="mx-auto max-w-7xl px-6">
            <div :class="effectiveCardWrapperClass">
                ...
                >
                    <!-- Carousel Viewport. No radius here: the cards already round themselves, and a
                         rounded clip box shaves the corner off every card that slides past the edge. -->
                    <div class="overflow-hidden">
                        <div
                            class="flex transition-transform duration-500 ease-out"
                            :style="{ transform: `translateX(-${currentIndex * (100 / effectiveColumns)}%` }"
                        >
                            <!-- Slide Card -->
                            <div
                                ...
                                :style="{ width: `${100 / effectiveColumns}%` }"
                            ...
                            >
                                <div :class="[isDark ? 'bg-surface-900/40 border border-surface-800/60' : cardBgClass('white')]" class="group relative overflow-hidden rounded-[2.25rem] p-2 shadow-xs transition-all duration-500 hover:-translate-y-1.5 hover:shadow-xl hover:shadow-primary-500/5 hover:border-primary-500/30 dark:hover:bg-surface-900/80">
                                    ...
                                    >
                                        ...
                                        <!-- Text Caption Overlay inside the image (cards only; the lightbox keeps its own) -->
                                        <div v-if="showCardCaptions && (item.title || item.description)" class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/45 to-transparent p-6 pt-16 flex flex-col justify-end transition-transform duration-500 group-hover:translate-y-1">
        ...
        <Teleport to="body">
            ...
            >
                <div
                    ...
                    class="frontend-theme-vars fixed inset-0 z-9999 flex items-center justify-center bg-black/90 p-4 backdrop-blur-xl md:p-10"
