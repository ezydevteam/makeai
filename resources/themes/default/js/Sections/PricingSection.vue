...
interface PricingCycle { amount: number; subtotal_amount: number; original_amount: number | null; formatted: string; subtotal_formatted: string; original_formatted: string | null; vat_percentage: number; vat_formatted: string; display_amount?: number; display_formatted?: string; uses_default: boolean; is_trial: boolean; trial_days: number | null }
...
const pricingDisplayPrice = (plan: PricingPlan) => {
    ...
    if (plan.pricing?.is_localized && c.display_formatted) return c.display_formatted
                    ...
                    <article v-for="plan in pricingSectionPlans()" :key="plan.id" :class="pricingPlanCardClass(plan)">
                        ...
                        <div class="mb-6">
                            <div class="flex flex-wrap items-end gap-2">
                                <span v-if="!plan.pricing?.is_localized && pricingActiveCycle(plan).original_formatted && Number(pricingActiveCycle(plan).original_amount) > pricingActiveCycle(plan).subtotal_amount" class="mb-1 text-lg font-bold text-gray-400 dark:text-gray-500 line-through">{{ pricingActiveCycle(plan).original_formatted }}</span>
