...
<template>
    <!-- Site-free base from the server, so an admin's category meta_title is honoured and
         the tab matches app.blade.php's <title inertia> instead of drifting from it. -->
    <Head :title="seo?.title_page || t(':name AI Tools', { name: category.name })" />

    <div class="w-full max-w-7xl mx-auto px-4 py-8 sm:px-6">
        <!-- Header -->
        <div :class="settings.category_enable_gradient ? 'bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white shadow-xl dark:from-blue-700 dark:via-indigo-850 dark:to-purple-900 border-0 p-8 sm:p-10 mb-10' : 'bg-gray-50 border border-gray-100 p-8 sm:p-10 mb-10 dark:bg-white/[0.02] dark:border-white/5'" class="rounded-2xl relative overflow-hidden">
            ...
            <div class="relative z-10 flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
                <div class="flex items-start gap-5 min-w-0 flex-1">
                    ...
                    <div class="min-w-0 flex-1">
                        <h1 :class="settings.category_enable_gradient ? 'modern-header-gradient-text font-black' : 'text-gray-900 dark:text-white'" class="text-3xl font-bold mb-2">{{ t(':name Tools', { name: category.name }) }}</h1>
                ...
                <div v-if="tools.length" class="shrink-0 w-full md:w-72 mt-2 md:mt-0">
                    <div class="relative">
                        ...
                        />
                        <i :class="isSearching ? 'ti ti-loader animate-spin text-primary-500' : (settings.category_enable_gradient ? 'category-gradient-search-icon ti ti-search' : 'text-gray-400 ti ti-search')" class="absolute left-3.5 top-1/2 -translate-y-1/2 text-base pointer-events-none z-10"></i>
                        <button
                            v-if="search"
                            type="button"
                            @click="search = ''"
        ...
        <template v-else>
            ...
            <div v-if="filteredTools.length" class="relative z-10" :class="{ 'modern-pagination-container': settings.category_enable_gradient }">
                ...
                <div v-if="settings.category_pagination === 'load_more' && currentPage < totalPages" class="mt-8 flex justify-center">
                    <button
                        ...
                        :class="[
                            settings.category_enable_gradient ? 'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 shadow-md shadow-blue-500/10' : 'bg-primary-500 hover:bg-primary-600',
            ...
            <!-- Search Empty State -->
            <div v-else-if="tools.length && !filteredTools.length" class="rounded-2xl mt-8 w-full border border-gray-200 bg-white p-12 text-center dark:border-white/5 dark:bg-white/[0.03]">
                <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-primary-550/10 text-primary-600 dark:bg-primary-500/10 dark:text-primary-300">
                    <i class="ti ti-search text-2xl"></i>
                </div>
                <h3 class="font-heading text-xl font-bold text-gray-900 dark:text-white mb-1">{{ t('No tools match your search') }}</h3>
                <p class="text-gray-500 dark:text-gray-400 text-sm mb-6">{{ t('Try checking for typos or searching a different term.') }}</p>
                <button type="button" @click="search = ''" :class="settings.category_enable_gradient ? 'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white shadow-md' : 'btn-primary'" class="rounded-full px-5 py-2.5 text-sm font-semibold transition">
                    {{ t('Clear search') }}
                </button>
            </div>
            ...
            <div v-else class="rounded-2xl mt-8 w-full border border-gray-200 bg-white p-12 text-center dark:border-white/5 dark:bg-white/[0.03]">
                <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-primary-550/10 text-primary-600 dark:bg-primary-500/10 dark:text-primary-300">
                    <i :class="[category.icon || 'ti ti-apps', 'text-2xl']"></i>
                </div>
                <h3 class="font-heading text-xl font-bold text-gray-900 dark:text-white mb-1">{{ t('No tools yet') }}</h3>
                <p class="text-gray-500 dark:text-gray-400 text-sm mb-6">{{ t("We're still building tools for this category. Check back soon!") }}</p>
                <Link :href="route('ai.tools.index')" :class="settings.category_enable_gradient ? 'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white shadow-md' : 'btn-primary'" class="inline-block rounded-full px-6 py-2.5 text-sm font-semibold transition">
                    {{ t('Explore All Tools') }}
                </Link>
            </div>
        </template>
