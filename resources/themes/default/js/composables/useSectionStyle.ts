...
export function useSectionStyle() {
    ...
    // Admins type "#pricing" as often as "pricing"; a leading # would render id="#pricing"
    // and break the very link it exists for, so strip it.
    const sectionAnchorId = (anchor: unknown): string | undefined => {
        const value = String(anchor ?? '').trim().replace(/^#+/, '')
        return value || undefined
    }

    const sectionVisibilityClass = (visibility: string | undefined, fallback = 'all'): string => ({
        all: '',
