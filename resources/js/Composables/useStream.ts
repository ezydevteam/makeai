                    ...
                    for (const event of events) {
                        ...
                        if (data.error) {
                            ...
                            if (isRetryable(data.error) && attempt < maxRetries) {
                                ...
                                // Abandon this stream and re-issue the request. A bare
                                // `break` here only left the event loop — the reader kept
                                // going, hit [DONE], and returned with no output, no error
                                // and no retry.
