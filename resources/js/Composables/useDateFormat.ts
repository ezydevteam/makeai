...
export function useDateFormat() {
    ...
    // formatWithTemplate reads a Date's LOCAL fields (getHours, getFullYear, …). Rebase
    // the instant so those fields report the SITE zone's wall clock, not the browser's —
    // this is what makes the whole team read one clock. The rebased Date is display-only
    // and never leaves here. formatRelative below stays on the true instant.
    const zoned = (date: string | Date) => asZonedDisplayDate(asDate(date), siteTimeZone())

    const formatDate = (date: string | Date): string =>
        formatWithTemplate(zoned(date), dateFormat.value, locale.value)

    const formatTime = (date: string | Date): string =>
        formatWithTemplate(zoned(date), timeFormat.value, locale.value)
