...
<template>
    <div class="space-y-4">
        <div v-for="(item, index) in items" :key="index" class="border-2 border-gray-100 rounded-3xl overflow-hidden transition-all" :class="{'border-primary-500 shadow-xl shadow-primary-600/5': item.isOpen}">
            <button 
                @click="toggle(index)"
                class="w-full flex items-center justify-between p-6 md:p-8 text-left bg-white hover:bg-gray-50 transition-colors"
            >
                <span class="text-lg font-black text-gray-900 leading-tight">{{ item.question }}</span>
            ...
            <div 
                v-show="item.isOpen" 
                class="p-6 md:p-8 pt-0 bg-white"
            >
