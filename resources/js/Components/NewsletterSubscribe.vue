...
<template>
    <div v-if="withCard" class="bg-primary-600 rounded-3xl p-8 md:p-12 text-white relative overflow-hidden shadow-2xl shadow-primary-500/30">
        ...
        <div class="relative z-10 max-w-2xl">
            <h2 class="text-3xl font-black mb-4">{{ t('Stay ahead with AI updates.') }}</h2>
            <p class="text-primary-100 mb-8 text-lg">{{ t('Join 10,000+ creators getting weekly AI prompts, news, and platform updates directly in their inbox.') }}</p>
            ...
            <form @submit.prevent="submit" :class="[
                ...
                <div class="flex-1">
                    <input v-model="form.email" type="email" :placeholder="t('your@email.com')" class="w-full bg-white/10 border border-white/20 rounded-2xl px-5 py-4 text-white placeholder-white/50 focus:bg-white focus:text-gray-900 focus:outline-none transition-all" required />
                </div>
            ...
            <p class="mt-4 text-xs text-primary-200">{{ t('No spam, ever. Unsubscribe with one click anytime.') }}</p>
        </div>
    </div>
