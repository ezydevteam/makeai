...
<template>
    <section class="mt-10 space-y-8">
        ...
        <div v-else id="comment-form" class="rounded-xl border border-gray-100 bg-gray-50 p-6 dark:border-surface-800 dark:bg-surface-900">
            ...
            <form v-else class="space-y-4" @submit.prevent="submit">
                <div v-if="replyingTo" class="flex items-center justify-between rounded-lg border border-primary-100 bg-white px-4 py-2 dark:border-primary-900/40 dark:bg-surface-800">
                    <span class="text-xs text-gray-500">{{ t('Replying to :name', { name: authorName(replyingTo) }) }}</span>
                ...
                <div class="flex gap-4">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex flex-wrap items-center gap-4">
                            <button type="button" class="flex items-center gap-1 text-xs font-bold transition-colors" :class="comment.liked ? 'text-primary-600' : 'text-gray-400 hover:text-primary-600'" @click="likeComment(comment)">
        ...
        <div v-if="reportingComment" class="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm">
            <form class="w-full max-w-md rounded-xl border border-gray-200 bg-white shadow-lg dark:border-surface-800 dark:bg-surface-900" @submit.prevent="submitReport">
                <div class="p-5">
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ t('Report comment') }}</h3>
                    <input v-model="reportForm.reason" type="text" class="mt-4 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" :placeholder="t('Reason')">
                    <textarea v-model="reportForm.details" rows="3" class="mt-3 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" :placeholder="t('Details')"></textarea>
                </div>
