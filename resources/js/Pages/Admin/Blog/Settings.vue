        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(0,0.9fr)]">
                ...
                <aside class="space-y-6">
                            ...
                            <div v-for="setting in booleanSettings.filter((setting) => ['default_allow_comments', 'show_reading_time', 'blog_sidebar', 'social_share_show_counts'].includes(setting.key))" :key="setting.key" class="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900/40">
                                <div class="flex items-center gap-3">
                                    <button
                                        ...
                                        :class="form[setting.key] ? 'bg-emerald-500 dark:bg-emerald-500' : 'bg-gray-300 dark:bg-gray-600'"
                                        @click="form[setting.key] = !form[setting.key]"
                                    >
                                        <span class="absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white shadow-sm transition-transform" :class="form[setting.key] ? 'translate-x-6' : 'translate-x-0'"></span>
                                    </button>
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    >
                                        <span
                                            ...
                                            :class="form[setting.key] ? 'translate-x-6' : 'translate-x-0'"
                                        ></span>
                                    </button>
                                    <div>
                    ...
                    <section class="rounded-2xl border border-primary-100 bg-primary-50/60 p-5 shadow-sm dark:border-primary-900/30 dark:bg-primary-900/10">
                        <div class="flex items-start gap-3">
                            <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-primary-600 shadow-sm dark:bg-gray-800 dark:text-primary-300">
                                <i class="ti ti-bulb text-lg"></i>
                            </div>
                            <div>
                                <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ t('Editing Note') }}</h2>
                                <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">{{ t('These defaults affect future blog posts and the public blog layout, but they do not overwrite every saved post individually.') }}</p>
                            </div>
                        </div>
                    </section>
                </aside>
