            ...
            <div v-for="theme in themes" :key="theme.slug" :class="[theme.is_active ? 'border-primary-500 ring-1 ring-primary-500/20' : 'border-gray-200']" class="relative bg-white border rounded-2xl overflow-hidden shadow-sm dark:bg-gray-900 dark:border-gray-800">
                ...
                <div class="p-5">
                    ...
                    <div v-if="theme.license?.buyer" class="mb-3 space-y-0.5 text-xs text-gray-500 dark:text-gray-400">
                        <p>{{ theme.license.license_type_label }} Â· {{ theme.license.buyer }}</p>
                    ...
                    <div class="flex items-center gap-2">
                        <button v-if="theme.is_premium && !theme.license_ok" @click="openLicense(theme)" class="px-4 py-2 btn-primary-admin text-sm font-medium rounded-lg flex-1">
                            {{ t('Verify License') }}
                        </button>
                        <button v-else-if="!theme.is_active && theme.license_ok" @click="activate(theme.slug)" :disabled="activating === theme.slug" class="px-4 py-2 btn-primary-admin text-sm font-medium rounded-lg flex-1 disabled:opacity-50">
                            <span v-if="activating === theme.slug" class="inline-flex items-center gap-2"><i class="ti ti-loader-2 animate-spin"></i>{{ t('Activating...') }}</span>
                            <span v-else>{{ t('Activate') }}</span>
                        </button>
                        <span v-else-if="theme.is_active" class="px-4 py-2 bg-gray-50 text-gray-500 text-sm font-medium rounded-lg flex-1 text-center dark:bg-gray-800">{{ t('Currently Active') }}</span>
                        <span v-else class="px-4 py-2 bg-gray-50 text-gray-400 text-sm rounded-lg flex-1 text-center dark:bg-gray-800">{{ t('Requires Extended License') }}</span>

                        <Link v-if="canOpenSettings(theme)" :href="route('admin.themes.settings', { slug: theme.slug })" class="px-3 py-2 bg-gray-50 text-gray-500 rounded-lg hover:bg-gray-100 hover:text-gray-900 transition-colors dark:bg-gray-800 dark:hover:bg-gray-700" :title="t('Settings')">
