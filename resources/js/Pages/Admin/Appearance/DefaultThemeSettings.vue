...
/** An active addon offering to render the homepage in place of the theme. */
...
const props = defineProps<{
    ...
    faqCategories?: { id: number; name: string }[]
    ...
    homepageProviders?: HomepageProvider[]
...
watch(activeTab, (tab) => {
    ...
    if (tab === 'presets') url.searchParams.delete('tab')
...
const faqCategoryOptions = computed(() => (props.faqCategories ?? []).map((category) => ({
    value: category.id,
    label: category.name,
})))
...
const titleSeparatorOptions = computed(() => {
    const site = String(page.props.appName || 'MakeAI')

    return ['|', '-', '–', '—', '·', '•', '/', '~'].map((separator) => ({
        value: separator,
        label: `${t('Page Title')} ${separator} ${site}`,
    }))
})
...
// Section flags that render as '0'/'1' selects were stored as booleans by older
// builds, so coerce rather than only filling in when absent.
const toFlagString = (value: unknown, fallback: '0' | '1'): '0' | '1' => {
    if (value === undefined || value === null || value === '') return fallback
    return normalizeBooleanValue(value, fallback === '1') ? '1' : '0'
}

// Section config keys backed by a switch. Naming is not a reliable guide — hero_gradient_enabled
// is a boolean while autoplay_enabled is a '0'/'1' select — so the set is listed explicitly.
const BOOLEAN_FLAG_KEYS = [
    'show_primary_cta', 'show_secondary_cta', 'show_stats', 'show_search_box',
    'show_hero_background', 'hero_gradient_enabled', 'show_hero_gradient_overlay',
    'show_rating', 'show_favorite', 'show_category', 'show_category_filter', 'show_search',
    'enable_card_style', 'show_stats_separator', 'show_brands',
    'show_button', 'show_description', 'show_read_more_btn',
    'hide_captions',
]
...
const themeForm = useForm({
    ...
    settings: {
        ...
        title_separator: resolvedThemeDefaults.value.title_separator ?? currentSettingString('title_separator', '|'),
...
const homepageForm = useForm({
    section: 'homepage',
    settings: {
        homepage_template: props.homepageTemplate ?? 'default',
...
/**
 * The homepage source picker only exists when an addon offers one — on a stock install
 * "custom" is the only possibility, and a one-option select is just noise.
 */
const homepageProviders = computed<HomepageProvider[]>(() => props.homepageProviders ?? [])
const hasHomepageProviders = computed(() => homepageProviders.value.length > 0)

const homepageTemplateOptions = computed(() => [
    { value: 'default', label: t('Custom (theme default)') },
    ...homepageProviders.value.map((provider) => ({ value: provider.value, label: provider.label })),
])

/** The addon currently serving the homepage, or null when the theme still owns it. */
const activeHomepageProvider = computed<HomepageProvider | null>(
    () => homepageProviders.value.find((provider) => provider.value === homepageForm.settings.homepage_template) ?? null,
)
...
const secCfg = (type: string): any => {
    ...
    // Configs saved alongside an image upload came back from multipart encoding with their
    // booleans stringified to "1"/"0". Repair them on read; re-saving then persists real
    // booleans. Excludes hide_controls/autoplay_enabled/carousel_autoplay, which are
    // genuinely '0'/'1' strings backing selects.
    for (const key of BOOLEAN_FLAG_KEYS) {
        if (key in cfg && typeof cfg[key] !== 'boolean') {
            cfg[key] = normalizeBooleanValue(cfg[key], false)
        }
    }

    if (type === 'hero') {
        // HeroSection renders the separator on by default, so seed it — otherwise the switch
        // reads as off while the line is actually showing.
        if (cfg.show_stats_separator === undefined) cfg.show_stats_separator = true
    }
    ...
    if (type === 'testimonials') {
        ...
        cfg.hide_controls = toFlagString(cfg.hide_controls, '0')
        cfg.autoplay_enabled = toFlagString(cfg.autoplay_enabled, '0')
        if (cfg.max_items === undefined) cfg.max_items = 6
        if (cfg.source === undefined) cfg.source = 'all'
    }
    if (type === 'stats_bar') {
    ...
    if (type === 'image_carousel') {
        ...
        cfg.carousel_autoplay = toFlagString(cfg.carousel_autoplay, '1')
...
// These sections only control how the block looks — the items themselves come from their own
// managers, so point admins there instead of leaving them hunting for a content field.
const contentManagedElsewhere: Record<string, { text: string; label: string; route: string }> = {
    pricing: {
        text: t('This section only controls how the pricing block looks. Plans, prices and features are managed in'),
        label: t('Premium → Plans'),
        route: 'admin.plans.index',
    },
    faq: {
        text: t('This section only controls how the FAQ block looks. Questions and answers are managed in'),
        label: t('Content → FAQs'),
        route: 'admin.faqs.index',
    },
    testimonials: {
        text: t('This section only controls how the testimonials block looks. The testimonials themselves are managed in'),
        label: t('Content → Testimonials'),
        route: 'admin.testimonials.index',
    },
}

// Mirrors HeroSection's default so the slider opens where the page actually renders.
const heroOverlayOpacity = computed((): number => {
    const raw = secCfg('hero').overlay_opacity
...
// licence + subscriptions enabled). Without it there is nothing to sell, so keep the section
// out of the builder entirely rather than letting an admin enable a dead section.
const orderedSections = ref<HomepageSectionDef[]>(
    HOMEPAGE_SECTION_DEFS.filter((sec) => sec.type !== 'pricing' || isProAvailable.value),
)
...
function saveHomepageSettings(): void {
    ...
    homepageForm.transform((data) => {
        ...
        return {
            ...
            // Multipart encoding stringifies booleans to "1"/"0", which silently corrupts
            // every section flag. Send the config as JSON so its types survive the upload.
            homepage_config: postOptions.forceFormData ? JSON.stringify(config) : config,
        }
...
// ─── Theme presets ───
const presets = computed<ThemePreset[]>(() => props.themePresets ?? [])
const isPresetModalOpen = ref(false)
const isApplyingPreset = ref(false)
const pendingPreset = ref<ThemePreset | null>(null)

// Which sections a preset overwrites, spelled out for the confirm dialog.
const sectionLabels: Record<string, string> = {
    theme: 'Colors & Typography',
    header: 'Header',
    footer: 'Footer',
    homepage: 'Homepage',
    homepage_config: 'Homepage Sections',
    tool_page: 'Page',
    custom_code: 'Custom Code',
}

const pendingPresetSections = computed(() =>
    (pendingPreset.value?.sections ?? []).map(s => t(sectionLabels[s] ?? s)).join(', '),
)
...
function handleConfirmPreset(): void {
    const preset = pendingPreset.value
    if (!preset) return
    isPresetModalOpen.value = false
    isApplyingPreset.value = true
    router.post(route('admin.themes.settings.apply-preset', { slug: props.theme.slug }), {
        preset: preset.id,
    }, {
        // Full reload so every other tab reflects the freshly applied settings.
        preserveScroll: true,
        onFinish: () => { isApplyingPreset.value = false },
    })
}
            ...
            <div class="min-w-0 flex-1">
                <div v-if="activeTab === 'presets'" class="space-y-6">
                    <section class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-surface-800 dark:bg-surface-900">
                        <div class="mb-5">
                            <h2 class="text-xs font-bold uppercase tracking-[0.18em] text-amber-700 dark:text-amber-300">{{ t('Theme Presets') }}</h2>
                        ...
                        <div v-else class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                            <button
                                ...
                                class="group relative flex flex-col overflow-hidden rounded-2xl border-2 bg-white text-left transition-all hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 disabled:opacity-60 dark:bg-surface-900"
                                :class="activePreset === preset.id
                                    ? 'border-primary-500 ring-2 ring-primary-500/30'
                                    : 'border-gray-200 hover:border-primary-300 dark:border-surface-700 dark:hover:border-primary-700'"
                                @click="choosePreset(preset)"
                            >
                                <!-- Mini mockup preview built from the preset's colors -->
                                <div
                                ...
                                >
                                    ...
                                    <div class="space-y-1.5 p-3">
                                        <div class="h-2.5 w-3/4 rounded-full" :style="{ backgroundColor: preset.preview.heading || '#111827' }"></div>
                                ...
                                <div class="flex flex-1 flex-col gap-1 border-t border-gray-100 p-4 dark:border-surface-800">
                                    <div class="flex items-center justify-between gap-2">
                                        ...
                                        >
                                            <i :class="preset.preview.mode === 'dark' ? 'ti ti-moon' : 'ti ti-sun'"></i>
                                ...
                                <!-- Radio indicator -->
                                <span
                                    class="absolute right-3 top-3 flex h-6 w-6 items-center justify-center rounded-full border-2 bg-white/90 shadow-sm backdrop-blur transition-colors dark:bg-surface-900/90"
                                    :class="activePreset === preset.id ? 'border-primary-500 text-primary-500' : 'border-gray-300 text-transparent group-hover:border-primary-400 dark:border-surface-600'"
                                >
                                ...
                                <span
                                    ...
                                    class="absolute left-3 top-3 rounded-full bg-primary-500 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-white shadow-sm"
                                >
                                    {{ t('Current') }}
                                </span>
                            </button>
                ...
                <div v-if="activeTab === 'general'" class="space-y-6">
                    ...
                    <section class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-surface-800 dark:bg-surface-900">
                        ...
                        <div class="grid gap-5 sm:grid-cols-2">
                            ...
                            />
                            <AppSelect
                                v-model="themeForm.settings.title_separator"
                                :label="t('Site title separator')"
                            ...
                            <div class="flex items-center gap-3 rounded-xl border border-gray-100 p-4 dark:border-surface-800">
                                <AppSwitch v-model="themeForm.settings.nav_progress_bar" />
                                <span class="text-sm font-semibold text-gray-800 dark:text-gray-200">{{ t('Show navigation progress bar') }}</span>
                            </div>
                            <div class="flex items-center gap-3 rounded-xl border border-gray-100 p-4 dark:border-surface-800">
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <div class="flex-1">
                                ...
                                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                                    {{ t('What visitors see at the site root. Choose an addon to hand your homepage over to it.') }}
                                </p>
                            </div>
                            <div class="min-w-[240px] shrink-0">
                                <AppSelect
                                    v-model="homepageForm.settings.homepage_template"
                                    :options="homepageTemplateOptions"
                                />
                            </div>
                        </div>
                    </section>
                    <section
                        v-if="activeHomepageProvider"
                        class="mt-4 flex items-start gap-2.5 rounded-xl border border-primary-100 bg-primary-50/60 px-4 py-6 text-sm leading-relaxed text-primary-800 dark:border-primary-900/40 dark:bg-primary-900/20 dark:text-primary-200"
                    >
                        <i class="ti ti-info-circle mt-0.5 shrink-0 text-sm"></i>
                        <span>
                            {{ activeHomepageProvider.description }}
                            <a
                                v-if="activeHomepageProvider.settings_url"
                                :href="activeHomepageProvider.settings_url"
                                class="font-semibold underline underline-offset-2 hover:no-underline"
                            >{{ t('Manage :addon settings', { addon: activeHomepageProvider.label }) }}</a>.
                        </span>
                    </section>

                    <template v-if="!activeHomepageProvider">
                        ...
                        <div v-if="!collapsedSections.has('hero')" class="p-5 space-y-4">
                            ...
                            <div class="grid gap-4 sm:grid-cols-2">
                                ...
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                    ...
                                    <p class="mt-1 text-xs text-gray-400">{{ t('Small pill shown above the headline. Leave empty to hide it.') }}</p>
                                </label>
                                <div>
                                    ...
                                    <AppSelect
                                        v-model="secCfg('hero').hero_vertical_padding"
                                        :placeholder="t('Variant default')"
                                        :options="[
                                            ...
                                            { value: 'full', label: t('Fullscreen') },
                                        ]"
                                    />
                                </div>
                                <div>
                                    <AppSelect
                                        v-model="secCfg('hero').hero_heading_size"
                                        :label="t('Headline Size')"
                                        :placeholder="t('Large (default)')"
                                        :options="[
                                ...
                                <div>
                                    <AppSelect
                                        v-model="secCfg('hero').hero_heading_color"
                                        :label="t('Headline Color')"
                                        :placeholder="t('Dark (default)')"
                                        :options="[
                                            { value: 'dark', label: t('Dark') },
                                            { value: 'primary', label: t('Primary') },
                                            { value: 'success', label: t('Success') },
                                    ...
                                    />
                                    <p class="mt-1 text-xs text-gray-400">{{ t('Ignored while a gradient or background image is active — the headline switches to a readable colour automatically.') }}</p>
                                </div>
                                <div>
                                    <AppSelect
                                        v-model="secCfg('hero').visibility"
                                        :label="t('Show On')"
                                ...
                                <div>
                                    <label for="section_anchor" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                        {{ t('Section Anchor') }}
                                        <input id="section_anchor" v-model="secCfg('hero').section_anchor" type="text" class="mt-1 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm dark:border-surface-700 dark:bg-surface-800 dark:text-white" />
                                    </label>
                                    <p class="mt-1 text-xs text-gray-400">{{ t('Optional id for linking straight to this section. Enter it without the #, e.g. home — then link to it as #home.') }}</p>
                                </div>
                            </div>
                            ...
                            <!-- Overlay -->
                            <div v-if="secCfgBool('hero', 'show_hero_background', false)" class="border-t border-gray-100 pt-4 dark:border-surface-700">
                                <div class="mb-1 flex items-center justify-between">
                                    <h4 class="text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ t('Darkening Overlay') }}</h4>
                                    <span class="text-xs font-semibold tabular-nums text-gray-500 dark:text-gray-400">{{ heroOverlayOpacity }}%</span>
                                </div>
                                <p class="mb-3 text-xs text-gray-400">{{ t('Darkens the background so the headline stays readable. Slide to 0 for no overlay.') }}</p>
                                <input
                                    ...
                                    max="100"
                                    step="5"
                                    class="overlay-slider h-2 w-full cursor-pointer rounded-full bg-gray-200 dark:bg-surface-700"
                                    :style="{ backgroundImage: `linear-gradient(to right, var(--color-primary-400) ${heroOverlayOpacity}%, transparent ${heroOverlayOpacity}%)` }"
                                    @input="secCfg('hero').overlay_opacity = Number(($event.target as HTMLInputElement).value)"
                                />
                            </div>
                            ...
                            <div v-if="homepageForm.settings.hero_variant !== 'tools-grid'" class="border-t border-gray-100 pt-4 dark:border-surface-700">
                                ...
                                <div v-if="secCfgBool('hero', 'show_stats', true)" class="space-y-3">
                                    <div class="flex items-center justify-between rounded-xl border border-gray-100 px-3 py-2 dark:border-surface-700">
                                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('Separator Line Above Stats') }}</span>
                                        <AppSwitch v-model="secCfg('hero').show_stats_separator" />
                                    </div>
                            ...
                            <div v-if="!collapsedSections.has(sec.type)" class="p-5 space-y-4">
                                ...
                                <div class="mt-4 pt-4 border-t border-gray-100 dark:border-surface-800">
                                    ...
                                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                        ...
                                        <div>
                                            <AppSelect
                                                v-model="secCfg(sec.type).visibility"
                                                :label="t('Show On')"
                                                :placeholder="t('All devices')"
                                                :options="[
                                                    { value: 'all', label: t('All devices') },
                                        ...
                                        <div>
                                            ...
                                            <input v-model="secCfg(sec.type).section_anchor" type="text" class="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm dark:border-surface-700 dark:bg-surface-800 dark:text-white" />
                                            <p class="mt-1 text-xs text-gray-400">{{ t('Optional id for linking straight to this section. Enter it without the #, e.g. pricing — then link to it as #pricing.') }}</p>
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                                                {{ t('Title Alignment') }}
                                ...
                                <!-- Image Carousel Section Options -->
                                <div v-if="sec.type === 'image_carousel'" class="mt-6 pt-6 border-t border-gray-100 dark:border-surface-800 space-y-4">
                                    <h4 class="text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">{{ t('Carousel Options') }}</h4>
                                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                                        <div>
                                            ...
                                            <AppSelect
                                                v-model="secCfg('image_carousel').carousel_layout"
                                                :options="[
                                        ...
                                        <div>
                                            ...
                                            <AppSelect
                                                v-model="secCfg('image_carousel').carousel_autoplay"
                                                :options="[
                                        ...
                                        <div>
                                            ...
                                            <input
                                                v-model="secCfg('image_carousel').carousel_time"
                                                type="number"
                                                min="1"
                                                max="60"
                                        ...
                                        <div>
                                            <div class="flex items-center justify-between gap-3 rounded-xl border border-gray-100 p-4 dark:border-surface-800">
                                                <div>
                                                    <span class="block text-sm font-semibold text-gray-900 dark:text-white">{{ t('Hide Title & Description') }}</span>
                                                    <span class="block text-xs text-gray-500 dark:text-gray-400">{{ t('Hides the caption overlay on the carousel images.') }}</span>
                                                </div>
                                                <AppSwitch v-model="secCfg('image_carousel').hide_captions" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div v-if="sec.type === 'testimonials'" class="mt-6 pt-6 border-t border-gray-100 dark:border-surface-800 space-y-4">
                                    <h4 class="text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">{{ t('Testimonials Options') }}</h4>
                                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                                        <div v-if="secCfg('testimonials').slider_columns !== '1'">
                                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                                                {{ t('Review Card Style') }}
                                            </label>
                                            <AppSelect
                                                v-model="secCfg('testimonials').card_style"
                                                :options="[
                                        ...
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                                                {{ t('Testimonial Slider Column') }}
                                            </label>
                                            <AppSelect
                                                v-model="secCfg('testimonials').slider_columns"
                                                :options="[
                                                    { value: '1', label: t('1 Column') },
                                                    { value: '2', label: t('2 Columns') },
                                                    { value: '3', label: t('3 Columns') },
                                        ...
                                        <div>
                                            ...
                                            <AppSelect
                                                v-model="secCfg('testimonials').hide_controls"
                                                :options="[
                                        ...
                                        <div>
                                            ...
                                            <AppSelect
                                                v-model="secCfg('testimonials').autoplay_enabled"
                                                :options="[
                                                    { value: '0', label: t('Disabled') },
                                                    { value: '1', label: t('Enabled') },
                                                ]"
                                ...
                                <!-- FAQ Section Options -->
                                <div v-if="sec.type === 'faq'" class="mt-6 pt-6 border-t border-gray-100 dark:border-surface-800 space-y-4">
                                    <div class="grid gap-4 sm:grid-cols-2">
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                                                {{ t('FAQ Categories') }}
                                            ...
                                            <AppSelect
                                                v-model="secCfg('faq').faq_categories"
                                ...
                                <div v-if="sec.type === 'stats_bar'" class="mt-6 pt-6 border-t border-gray-100 dark:border-surface-800 space-y-6">
                                    ...
                                    <div v-if="secCfgBool('stats_bar', 'show_stats', true)" class="space-y-4">
                                        <div class="sm:max-w-xs">
                                            <AppSelect
                                                v-model="secCfg('stats_bar').stats_style"
                                                :label="t('Stats Style')"
                                            ...
                                            />
                                            <p class="mt-1 text-xs text-gray-400">{{ t('Colours the stat numbers, with the matching shade applied to their labels.') }}</p>
                                        </div>
                                                ...
                                                <div class="sm:col-span-2">
                                                    ...
                                                    <div v-if="brand.image || brandLogoPreviewUrls[Number(idx)]" class="mt-2 flex items-center gap-3">
                                                        <div class="h-8 max-w-[120px] rounded border border-gray-200 bg-white p-1 flex items-center justify-center dark:border-surface-700 dark:bg-surface-900">
                                                            <img :src="brandLogoPreviewUrls[Number(idx)] || mediaUrl(brand.image)" class="h-full object-contain" />
                                                        </div>
                                ...
                                <div v-if="sec.type === 'newsletter'" class="mt-6 pt-6 border-t border-gray-100 dark:border-surface-800 space-y-6">
                                    <div class="grid gap-4 sm:grid-cols-2">
                                        ...
                                        <div>
                                            <IconClassSelect v-model="secCfg('newsletter').button_icon" :label="t('Button Icon')" />
                                            <p class="mt-1 text-xs text-gray-400">{{ t('Clear the Button Text above to show the icon on its own.') }}</p>
                                        </div>
                                    </div>
                                </div>
                                                ...
                                                <div class="sm:col-span-3">
                                                    ...
                                                    <div v-if="item.image_url || carouselItemPreviewUrls[item._key || '']" class="mt-2 flex items-center gap-3">
                                                        <div class="h-12 w-20 rounded border border-gray-200 bg-white p-1 flex items-center justify-center dark:border-surface-700 dark:bg-surface-900">
                                                            <img :src="carouselItemPreviewUrls[item._key || ''] || mediaUrl(item.image_url)" class="h-full w-full object-cover rounded" />
                                                        </div>
                                ...
                                <!-- Sections whose content lives in a dedicated manager -->
                                <div
                                    v-if="contentManagedElsewhere[sec.type]"
                                    class="mt-6 flex items-start gap-2.5 rounded-xl border border-primary-100 bg-primary-50/60 px-4 py-3 text-xs leading-relaxed text-primary-800 dark:border-primary-900/40 dark:bg-primary-900/20 dark:text-primary-200"
                                >
                                    <i class="ti ti-info-circle mt-0.5 shrink-0 text-sm"></i>
                                    <span>
                                        {{ contentManagedElsewhere[sec.type].text }}
                                        <Link :href="route(contentManagedElsewhere[sec.type].route)" class="font-semibold underline underline-offset-2 hover:no-underline">{{ contentManagedElsewhere[sec.type].label }}</Link>.
                                    </span>
                                </div>
                            </div>
                        </section>
                ...
                <div v-else-if="activeTab === 'custom_code'" class="space-y-6">
                    <section class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-surface-800 dark:bg-surface-900">
                        ...
                        <div class="space-y-5">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                {{ t('Custom CSS') }}
                                <textarea v-model="customCodeForm.settings.custom_css" rows="12" class="mt-2 w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-mono dark:border-surface-700 dark:bg-surface-800 dark:text-white"></textarea>
                            </label>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                {{ t('Custom Header Code') }}
                                <textarea v-model="customCodeForm.settings.custom_header_code" rows="10" class="mt-2 w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-mono dark:border-surface-700 dark:bg-surface-800 dark:text-white"></textarea>
                            </label>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                {{ t('Custom Footer Code') }}
                                <textarea v-model="customCodeForm.settings.custom_footer_code" rows="10" class="mt-2 w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-mono dark:border-surface-700 dark:bg-surface-800 dark:text-white"></textarea>
                            </label>
                        </div>
                    </section>
                </div>
...
<style scoped>
/*
 * The filled portion comes from a gradient on the input's own background, sized inline from
 * the current value. Both engines paint a native track over that background, so the tracks
 * are cleared to transparent to let it through — which also means the thumb has to be styled
 * by hand, since accent-color no longer applies.
 */
.overlay-slider {
