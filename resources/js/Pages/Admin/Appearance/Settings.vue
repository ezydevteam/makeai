...
<template>
    ...
    <div class="max-w-6xl mx-auto px-6 py-8">
        <div class="mb-8">
            ...
            <p class="text-sm text-gray-500 mt-1">Customize the visual identity of your admin panel and frontend.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div class="space-y-6">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <h3 class="font-black text-gray-900 uppercase tracking-widest">Admin Panel Appearance</h3>
                </div>
                <form @submit.prevent="updateAdmin" class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase mb-3 tracking-widest">Brand Color</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase mb-3 tracking-widest">Sidebar Background</label>
                        <select v-model="adminForm.settings.sidebar_bg" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none">
                            <option value="#ffffff">Clean White</option>
                            <option value="#f9fafb">Light Gray</option>
                            <option value="#111827">Modern Dark</option>
                        </select>
                    </div>
                    <button type="submit" :disabled="adminForm.processing" class="w-full py-4 bg-gray-900 text-white rounded-2xl font-bold hover:bg-gray-800 transition-all shadow-xl shadow-gray-900/10">SAVE ADMIN UI</button>
                </form>
            </div>
            ...
            <div class="space-y-6">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <h3 class="font-black text-gray-900 uppercase tracking-widest">Frontend Theme Settings</h3>
                </div>
                <form @submit.prevent="updateTheme" class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase mb-3 tracking-widest">Primary Color</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase mb-3 tracking-widest">Accent Color</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase mb-3 tracking-widest">Border Radius</label>
                        <select v-model="themeForm.settings.border_radius" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none">
                            ...
                            <option value="20px">Modern (20px)</option>
                            <option value="999px">Pill (999px)</option>
                        </select>
                    </div>
                    <button type="submit" :disabled="themeForm.processing" class="w-full py-4 bg-primary-600 text-white rounded-2xl font-bold hover:bg-primary-500 transition-all shadow-xl shadow-primary-600/10">SAVE THEME UI</button>
                </form>
            </div>
        </div>
    </div>
