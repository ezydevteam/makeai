...
interface Reply { id: number; author_type: string; author_name: string; content: string; is_internal_note: boolean; is_ai_draft: boolean; attachments?: { name: string; url: string }[]; created_at: string }
...
const trackCannedUsage = (id: number) => {
    fetch(route('admin.support.canned-responses.track', id), { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector<HTMLMetaElement>('meta[name="csrf-token"]')?.content ?? '' } })
}
    ...
    <div class="mx-auto max-w-7xl px-6 py-8">
        ...
        <div class="grid gap-6 xl:grid-cols-[1fr_360px]">
            <section class="space-y-4">
                <article v-for="reply in ticket.replies" :key="reply.id" :class="[reply.is_internal_note ? 'border-amber-300 bg-amber-50' : reply.is_ai_draft ? 'border-violet-300 bg-violet-50 dark:bg-violet-950/20 dark:border-violet-800' : 'border-gray-200 bg-white dark:bg-surface-900 dark:border-surface-800']" class="rounded-xl border p-5 shadow-sm">
                    <div class="mb-3 flex items-center justify-between">
                        ...
                        <span class="rounded-full px-2 py-1 text-xs font-medium" :class="reply.is_ai_draft ? 'bg-violet-100 text-violet-700' : reply.is_internal_note ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600'">{{ reply.is_ai_draft ? t('AI draft') : reply.is_internal_note ? t('Internal note') : t(reply.author_type) }}</span>
                    </div>
                ...
                <form @submit.prevent="sendReply" class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-surface-800 dark:bg-surface-900">
                    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <select @change="insertCanned(($event.target as HTMLSelectElement).value); trackCannedUsage(parseInt(($event.target as HTMLSelectElement).selectedOptions[0]?.dataset?.id ?? '0'))" class="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm dark:border-surface-700 dark:bg-surface-800 dark:text-white">
                                <option value="">{{ t('Insert canned response') }}</option>
                                <option v-for="response in cannedResponses" :key="response.id" :value="response.content" :data-id="response.id">{{ response.title }}</option>
                            </select>
                    ...
                    <button type="submit" :disabled="form.processing" class="mt-4 rounded-lg btn-primary disabled:opacity-60">{{ form.processing ? t('Sending...') : t('Send') }}</button>
                </form>
            </section>
            ...
            <aside class="space-y-4">
                <form @submit.prevent="updateState" class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-surface-800 dark:bg-surface-900">
                    ...
                    <button type="submit" class="mt-5 w-full rounded-lg btn-primary">{{ t('Update Ticket') }}</button>
                </form>
            </aside>
        </div>
    </div>
