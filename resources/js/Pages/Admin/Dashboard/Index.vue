...
const props = defineProps<{
    ...
    dashboardCharts: {
        ...
        activeUsersChart: Record<ChartPeriod, TimeSeriesPoint[]>
...
// The sixth grid slot holds Revenue when billing is available (Extended license
// AND subscriptions on). Otherwise it falls back to Open Tickets, or to Active
// Users when the ticket system is switched off — so the row is never short a card.
const showRevenueSubstitute = computed(() => !isProAvailable.value)
...
const periodSeries = computed(() => {
    ...
    return {
        ...
        activeUsers: getSeriesValues(getPeriodSeries(dashboardCharts.value.activeUsersChart, period)),
...
const cards = computed<StatSeriesCard[]>(() => {
    ...
    return [
        ...
        {
            // openTicketsChart is a CUMULATIVE series, so the headline figure is the
            // last point (the running total), not the sum of every point.
            title: 'Open Tickets',
            value: formatRangeNumber(lastSeriesValue(series.openTickets)),
            sparkline: series.openTickets,
            ...comparisonProps('openTickets'),
            color: 'success',
            icon: 'ticket',
            metric: 'openTickets',
            visible: showRevenueSubstitute.value && ticketsEnabled.value,
        },
        {
            // Fallback for the same slot when the ticket system is switched off.
            // Per-bucket series (one bucket per user, keyed on last_login_at), so
            // unlike Open Tickets this one sums.
            title: 'Active Users',
            value: formatRangeNumber(sumSeries(series.activeUsers)),
            sparkline: series.activeUsers,
            ...comparisonProps('activeUsers'),
            color: 'success',
            icon: 'users',
            metric: 'activeUsers',
            visible: showRevenueSubstitute.value && !ticketsEnabled.value,
...
<template>
    ...
    <div class="w-full space-y-6 px-4 sm:px-6 lg:px-6 xl:px-8 2xl:px-10">
        ...
        <!-- Demo Mode Indicator -->
        <div v-if="isDemo" class="rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900/30 dark:bg-amber-900/20">
