                ...
                <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
                    <div v-for="setting in settings" :key="setting.key" class="rounded-2xl border border-gray-200 bg-white p-5 shadow-card transition hover:border-primary-200 hover:shadow-md dark:border-surface-700 dark:bg-surface-900">
                        ...
                        <div v-if="setting.type === 'boolean'" class="flex items-center justify-between gap-4 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 dark:border-surface-700 dark:bg-surface-800">
                            <div>
                                <p class="text-sm font-medium text-gray-700 dark:text-gray-200">{{ resolveBoolean(form[setting.key]) ? t('Enabled') : t('Disabled') }}</p>
                                <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('Toggle this option on or off.') }}</p>
                            </div>
                            <button type="button" @click="form[setting.key] = !resolveBoolean(form[setting.key])" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors" :class="resolveBoolean(form[setting.key]) ? 'bg-primary-500' : 'bg-gray-300 dark:bg-surface-700'">
                                <span class="inline-block h-5 w-5 rounded-full bg-white shadow-sm transition-transform" :class="resolveBoolean(form[setting.key]) ? 'translate-x-5' : 'translate-x-0.5'"></span>
                            </button>
                        </div>
