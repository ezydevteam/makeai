...
// Subscription/billing features (plans, the purchasable credit wallet) only apply
// when Pro is available; in quota mode the daily/monthly limits are the governance.
const isProAvailable = computed(() => Boolean(page.props.isProAvailable));
// Referral earnings only mean anything when the affiliate program is enabled.
...
const form = useForm({
    ...
    phone_country: props.user.phone_country ?? (props.user.country ?? 'US'),
    timezone: props.user.timezone ?? 'UTC',
    daily_limit: props.user.daily_limit ?? '',
    monthly_limit: props.user.monthly_limit ?? '',
    password: '',
...
// SMS delivery is only offered when an SMS gateway is active AND the user has a
// verified phone number (see user_can_receive_sms()).
const deliverViaOptions = computed(() => {
    const options = [
    ...
    if (props.smsDeliveryAvailable) {
        options.push({ value: 'sms', label: t('Text message (SMS) only') });
        options.push({ value: 'in_app_sms', label: t('In-app + Text message') });
    }
...
const sendNotification = () => {
    // The picker holds local wall clock; the server schedules on an instant. Without
    // this the notification fires hours from when the admin asked for it.
    notificationForm.transform((data) => ({
        ...data,
        scheduled_at: fromDateTimeLocalInput(data.scheduled_at),
    }));
...
const logoutAllSessions = () => {
    logoutForm.post(route('admin.users.logout-sessions', props.user.ulid), {
        preserveScroll: true,
...
<template>
    ...
    <div class="mx-auto w-full sm:max-w-5xl px-6">
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <PhoneInput
                                    v-model="form.phone"
                                    v-model:country="form.phone_country"
                                    :label="t('Phone Number')"
                                    :error="form.errors.phone"
                                />
                                <div class="mt-1">
                                    <span v-if="user.phone_verified_at" class="inline-flex items-center gap-1.5 text-xs font-medium text-green-600 dark:text-green-400">
                                        <i class="ti ti-circle-check-filled"></i>
                                        {{ t('Verified') }} · {{ formatDateTime(user.phone_verified_at) }}
                                    </span>
                                    <span v-else-if="user.phone" class="inline-flex items-center gap-1.5 text-xs font-medium text-amber-600 dark:text-amber-400">
                            ...
                            <div>
                                <AppSelect v-model="form.timezone" :options="timezones" :label="t('Timezone')" :placeholder="t('Select Timezone')" live-search />
                            </div>
                            <div v-if="isProAvailable">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ t('Daily Limit') }}</label>
                                <input v-model="form.daily_limit" type="number" step="0.01" min="0" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-900 focus:border-primary-500 focus:outline-none dark:bg-surface-800 dark:border-gray-700 dark:text-white" :placeholder="t('Global default')" />
                                <p v-if="form.errors.daily_limit" class="mt-1 text-xs text-red-500">{{ form.errors.daily_limit }}</p>
                                <p v-else class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ t('Credits/day. Blank = global default, 0 = unlimited.') }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ t('Monthly Limit') }}</label>
                                <input v-model="form.monthly_limit" type="number" step="0.01" min="0" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-900 focus:border-primary-500 focus:outline-none dark:bg-surface-800 dark:border-gray-700 dark:text-white" :placeholder="t('Global default')" />
                                <p v-if="form.errors.monthly_limit" class="mt-1 text-xs text-red-500">{{ form.errors.monthly_limit }}</p>
                                <p v-else class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ t('Credits/month. Blank = global default, 0 = unlimited.') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="pt-4 border-t border-gray-100 dark:border-gray-800">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        <input v-model="form.password_confirmation" :type="showPasswordConfirmation ? 'text' : 'password'" :placeholder="t('Re-type password')" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 pr-11 text-sm text-gray-900 focus:border-primary-500 focus:outline-none dark:bg-surface-800 dark:border-gray-700 dark:text-white" />
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden dark:bg-surface-900 dark:border-gray-800">
                    ...
                    <div class="divide-y divide-gray-100 dark:divide-gray-800">
                        <div v-for="log in user.login_history" :key="log.id" class="px-6 py-2">
                    ...
                    <div v-if="canAny(['users.edit', 'users.manage'])" class="px-6 py-4 border-t border-gray-100 dark:border-gray-800">
                        <button
                            ...
                            class="w-full inline-flex items-center justify-center gap-2 py-2.5 bg-red-50 border border-red-200 text-red-700 font-bold text-sm rounded-xl hover:bg-red-100 dark:bg-red-950/20 dark:border-red-900/30 dark:text-red-400 dark:hover:bg-red-900/30 transition-colors disabled:opacity-50"
                        >
                            <i class="ti ti-logout-2 text-base"></i>
                            {{ logoutForm.processing ? t('Signing out...') : t('Sign out all sessions') }}
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div class="hidden lg:block overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        <thead class="bg-gray-50 border-b border-gray-100 dark:bg-surface-850 dark:border-gray-800">
                            <tr>
                                <th class="px-6 py-3 text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">{{ t('Tool') }}</th>
                                <th class="px-6 py-3 text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">{{ t('Model') }}</th>
                                <th class="px-6 py-3 text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">{{ t('Provider') }}</th>
                                <th class="px-6 py-3 text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">{{ t('Tokens') }}</th>
                                <th class="px-6 py-3 text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">{{ t('Credits') }}</th>
                                <th class="px-6 py-3 text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">{{ t('Status') }}</th>
                                <th class="px-6 py-3 text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">{{ t('Date') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            <tr v-for="item in usageHistory" :key="item.id" class="hover:bg-gray-50/60 dark:hover:bg-gray-800/40 transition-colors">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ formatDateTime(item.created_at) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <ActionConfirmModal
            ...
            :title="t('Log out all sessions?')"
            :message="t('This immediately signs :name out of every device and browser session. They will need to log in again.', { name: user.name })"
            :confirm-label="t('Log out all sessions')"
            :processing-label="t('Signing out...')"
        ...
        <ActionConfirmModal
            ...
            :message="user.is_banned
                ...
                : t('Are you sure you want to ban :name? This blocks their access across the entire application — login, AI usage, comments, reviews and support — immediately.', { name: user.name })"
        ...
        >
            <div class="space-y-4 text-left">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <AppSelect v-model="notificationForm.deliver_via" :label="t('Deliver via')" :options="deliverViaOptions" />
                </div>
                <label class="block">
