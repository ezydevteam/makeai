...
async function finalizeInstall() {
    ...
    try {
        // Inertia POST — browser handles redirect on success
        router.post(route('install.finalize'), {}, {
            preserveScroll: true,
            preserveState: false,
            onError: () => {
                installError.value = page.props.flash?.error ?? t('Installation failed. Check the logs for details.')
                installing.value = false
            },
            onFinish: () => {
                // If no redirect happened, we're still on the page (error case)
                setTimeout(() => {
                    installing.value = false
                }, 1000)
            },
        ...
        installError.value = t('Installation failed. Please try again.')
        installing.value = false
    }
}
</script>
