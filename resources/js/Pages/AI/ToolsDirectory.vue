        ...
        <div class="relative mx-auto max-w-7xl px-4 py-8 sm:px-6">
            ...
            <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center lg:items-end lg:ml-auto w-full lg:w-auto">
                    <div class="relative w-full sm:w-64">
                        <i class="ti ti-search absolute left-3 top-1/2 -translate-y-1/2 text-base text-gray-400 pointer-events-none"></i>
                        <input
                            v-model="search"
                            type="text"
            ...
            <div v-else-if="filtered.length && viewMode === 'list'" class="flex flex-col gap-3">
                <template v-for="(item, index) in paginatedTools" :key="'list-'+item.id">
                    ...
                    <Link
                        :href="route('ai.tools.show', item.slug)"
                        class="group relative flex flex-col gap-4 rounded-2xl border border-gray-200 bg-white p-4 transition-all hover:!border-primary-200 hover:shadow-md dark:border-white/5 dark:bg-white/[0.03] sm:flex-row sm:items-center sm:justify-between"
                    >
            ...
            <div v-if="filtered.length" class="relative z-10" :class="{ 'modern-pagination-container': settings.archive_layout === 'modern' }">
                ...
                <div v-if="settings.archive_pagination === 'load_more' && currentPage < totalPages" class="mt-8 flex justify-center">
                    <button
                        ...
                        :class="[
                            settings.archive_layout === 'modern' ? 'bg-gradient-to-r from-primary-500 to-violet-600 hover:from-primary-600 hover:to-violet-700 shadow-md shadow-primary-500/10' : 'bg-primary-500 hover:bg-primary-600',
            ...
            <div v-else class="card mx-auto mt-8 max-w-2xl border border-gray-200 bg-white p-8 text-center dark:border-white/5 dark:bg-white/[0.03]">
                ...
                <button @click="search = ''; activeCategory = 'all'; activeTag = null" :class="settings.archive_layout === 'modern' ? 'bg-gradient-to-r from-primary-500 to-violet-600 hover:from-primary-600 hover:to-violet-700 text-white shadow-md' : 'btn-primary'" class="mt-5 rounded-full px-4 py-2 text-sm font-semibold transition">
                    {{ t('Clear Filters') }}
                </button>
            </div>
        </div>
