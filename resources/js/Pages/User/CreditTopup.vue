...
const submit = () => {
    ...
    router.post(route('user.dashboard.credit-topup.checkout'), {
                ...
                <div class="space-y-6">
                    ...
                    <section class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-surface-800 dark:bg-surface-900">
                        ...
                        <div class="relative mb-4">
                            <span class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4 text-lg font-bold text-gray-400">$</span>
                            <input
                                v-model="amount"
                                type="number"
                                min="0"
                    ...
                    <section v-if="numericAmount > 0" class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-surface-800 dark:bg-surface-900">
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="font-semibold text-gray-900 dark:text-white">${{ creditTopup.price_per_unit.toFixed(4) }} = 1 {{ t('credit') }}</span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                        ...
                        <div class="space-y-2">
                            ...
                            >
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                    {{ t('Spend :amount or more', { amount: '$' + tier.min_amount }) }}
                                </span>
                        ...
                        <div v-if="nextBonusTier && numericAmount > 0" class="mt-3 rounded-lg border border-dashed border-primary-200 bg-primary-50/50 p-3 dark:border-primary-800 dark:bg-primary-900/10">
                            <p class="text-xs font-semibold text-primary-700 dark:text-primary-300">
                                {{ t('Add :more more to unlock :percent% bonus credits!', {
                                    more: '$' + (nextBonusTier.min_amount - numericAmount).toFixed(2),
                    ...
                    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-surface-800 dark:bg-surface-900">
                        ...
                        <div class="space-y-3 text-sm font-medium text-gray-600 dark:text-gray-400">
                            <div class="flex justify-between gap-4">
                                <span>{{ t('Amount') }}</span>
                                <span class="font-bold text-gray-900 dark:text-white">${{ numericAmount.toFixed(2) }}</span>
                            </div>
                            <div class="flex justify-between gap-4">
                            ...
                            <div v-if="processingFee > 0" class="flex justify-between gap-4">
                                ...
                                <span class="font-bold text-gray-900 dark:text-white">${{ processingFee.toFixed(2) }}</span>
                            </div>
                        </div>
                        ...
                        <div v-if="numericAmount > 0" class="mt-1 flex items-end justify-between gap-4">
                            ...
                            <span class="text-lg font-black text-gray-900 dark:text-white">${{ totalAmount.toFixed(2) }}</span>
                        </div>
