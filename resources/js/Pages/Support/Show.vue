...
const reply = () => {
    form.post(route('support.tickets.reply', props.ticket.ticket_number), {
        forceFormData: true,
        preserveScroll: true,
...
<template>
    ...
    <div class="mx-auto max-w-5xl px-6 py-10">
        <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <Link :href="route('support.index')" class="text-sm font-medium text-primary-600 hover:text-primary-500">{{ t('Back to support') }}</Link>
        ...
        <div class="space-y-4">
            <article v-for="replyItem in ticket.replies" :key="replyItem.id" :class="replyItem.author_type === 'admin' ? 'border-primary-300' : 'border-gray-200'" class="rounded-xl border bg-white p-5 shadow-sm dark:border-surface-800 dark:bg-surface-900">
                ...
                <div class="prose prose-sm max-w-none dark:prose-invert" v-html="sanitize(replyItem.content)"></div>
        ...
        <form v-if="ticket.status !== 'closed'" @submit.prevent="reply" class="mt-6 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-surface-800 dark:bg-surface-900">
            ...
            <input type="file" multiple @change="setFiles" class="mt-4 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm dark:border-surface-700 dark:bg-surface-800 dark:text-white">
