...
<template>
    ...
    <div class="auth-page">
        <div class="auth-glow">
            <div class="absolute inset-0 bg-gradient-to-br from-gray-50 via-accent-50/20 to-white"></div>
            <div class="absolute top-1/4 left-1/3 w-80 h-80 bg-accent-100/40 rounded-full blur-3xl"></div>
            <div class="absolute bottom-1/3 right-1/4 w-64 h-64 bg-primary-100/30 rounded-full blur-3xl"></div>
        </div>
        ...
        <div class="w-full max-w-md relative z-10">
            ...
            <div class="text-center mb-10">
                ...
                <h1 class="text-3xl font-black text-gray-900 tracking-tight">{{ t('Create Account') }}</h1>
                <p class="text-gray-500 mt-2 text-sm font-medium">{{ t('Join thousands of creators using :app', { app: appName }) }}</p>
            </div>
            ...
            <div class="auth-card">
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    <div>
                        <label for="name" class="auth-label">{{ t('Full Name') }}</label>
                        <input id="name" v-model="form.name" type="text" required autofocus autocomplete="name" class="auth-input" :placeholder="t('John Doe')" />
                        <p v-if="form.errors.name" class="auth-error">{{ form.errors.name }}</p>
                    </div>

                    <div>
                        <label for="email" class="auth-label">{{ t('Email Address') }}</label>
                        <input id="email" v-model="form.email" type="email" required autocomplete="email" class="auth-input" :placeholder="t('name@company.com')" />
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input id="password" v-model="form.password" :type="showPassword ? 'text' : 'password'" required autocomplete="new-password" class="auth-input pr-12" :placeholder="t('Min. 8 characters')" />
                    ...
                    <div>
                        ...
                        <input id="password_confirmation" v-model="form.password_confirmation" type="password" required autocomplete="new-password" class="auth-input" :placeholder="t('••••••••')" />
                    </div>
                    ...
                    <button type="submit" :disabled="form.processing" class="auth-btn">
                        ...
                        <span>{{ form.processing ? t('Creating account...') : t('Create Account') }}</span>
                    </button>
                </form>
