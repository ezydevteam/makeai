...
> **Loaded every session.** This is the complete system prompt for all development work on MakeAI.
...
---
## Use **actionconfirm component** for any action confirm instead of browser built-in alert.
...
**Provider Names (SDK-native):** openai, anthropic, gemini, deepseek, xai, openrouter, groq, mistral, ollama, azure, bedrock, cohere, eleven, jina, voyageai

**CRITICAL:** ALL AI operations go through `AiService` → `ProviderRegistry` → `LaravelAiDriver`. Never call raw HTTP or SDK providers directly.
