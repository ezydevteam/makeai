...
class ExportUserDataJob implements ShouldQueue
{
    ...
    public function handle(InAppNotificationService $notifications, MailService $mail): void
    {
        ...
        try {
            ...
            $this->notifyReady($notifications, $mail, $expiresAt);
        } catch (\Throwable $e) {
    ...
    /**
     * The export is only downloadable for a 24h window, so the user has to be
     * told it finished — until now the row just flipped to 'ready' and they had
     * to happen to reload the privacy page to find out.
     *
     * A notification failure must not fail the job: the file is built and the
     * request is already marked ready, and retrying would rebuild the whole zip.
     */
    private function notifyReady(InAppNotificationService $notifications, MailService $mail, \Illuminate\Support\Carbon $expiresAt): void
    {
        $downloadUrl = route('user.dashboard.privacy.export.download', $this->exportRequest);

        try {
            $notifications->send($this->user, [
                'title' => translate('Your data export is ready'),
                'message' => translate('Your data export has finished processing and is ready to download.'),
                'level' => 'success',
                'category' => 'export',
                'icon' => 'ti ti-download',
                'action_url' => $downloadUrl,
                'action_label' => translate('Download'),
                'meta' => ['export_id' => $this->exportRequest->id],
            ]);

            if ($this->user->email) {
                $mail->send('export_ready', $this->user->email, [
                    'user_name' => $this->user->name,
                    'export_name' => basename($this->exportRequest->file_path ?? 'export.zip'),
            ...
            Log::warning('ExportUserDataJob: ready notification failed', [
                'user_id' => $this->user->id,
                'export_id' => $this->exportRequest->id,
                'error' => $e->getMessage(),
            ]);
