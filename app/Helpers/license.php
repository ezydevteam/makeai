...
if (! function_exists('coupons_enabled')) {
    /**
     * Check if the discount-coupon system should be available.
     * Requires: Extended License AND coupons enabled by admin (default on).
     *
     * Gates the storefront coupon field, the header coupon banner, server-side
     * coupon validation, and the admin Coupons screen.
     */
    function coupons_enabled(): bool
    {
        return is_extended_license() && (bool) settings('coupons_enabled', true);
    }
}
