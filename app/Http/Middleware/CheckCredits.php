...
class CheckCredits
{
    public function handle(Request $request, Closure $next)
    {
        ...
        $model = $this->resolveModel($request, $template);
    ...
    /**
     * Resolve the model this request will actually bill against.
     *
     * Must mirror PromptBuilder::build — an explicit, admin-activated model wins,
     * then the tool's override, then the global default. A model smuggled through
     * the `fields` array is ignored (as GenerateController ignores it), so a cheap
     * slug there cannot shrink the pre-flight estimate below what the request will
     * really cost and slip past the balance check.
     */
    private function resolveModel(Request $request, ?AiTool $template): string
    {
        $requested = $request->input('model');

        if (is_string($requested) && $requested !== '' && AiModel::where('slug', $requested)->active()->exists()) {
            return $requested;
        }
