...
class AdController extends Controller
{
    ...
    private function payload(AdRequest $request): array
    {
        ...
        if ($request->hasFile('image_file')) {
            ...
            // Store the RELATIVE disk key; the view resolves it with mediaUrl(). Persisting
            // Storage::url() baked a "/storage/..." path into the row that breaks on cloud.
            $validated['image_url'] = store_public_upload($request->file('image_file'), 'ads');
    ...
    /**
     * Delete a replaced ad image. media_path() reduces whichever format the row holds —
     * a relative key, a legacy "/storage/..." URL, or a bucket/CDN URL — to the disk key,
     * so old rows keep working. An external URL (an ad pointing at a remote image) is not
     * on our disk and is left alone.
     */
    private function deleteUploadedAdImage(?string $imageUrl): void
    {
        $key = media_path($imageUrl);
