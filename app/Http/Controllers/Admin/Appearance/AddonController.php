...
class AddonController extends Controller
{
    ...
    public function addonSettings(string $slug)
    {
        ...
        // with bespoke props (e.g. ai-knowledge-base passes `providers`). Rendering
            ...
            foreach ($config['settings'] as $setting) {
                if (($setting['type'] ?? '') === 'file' && ! $request->hasFile($setting['key'])) {
                    ...
                    if ($rawVal === null || $rawVal === '') {
                        ...
                        if (is_string($val)) {
                            // The client may echo back a resolved URL ("/storage/x" on local,
                            // a bucket/CDN URL on cloud). Reduce it to the relative disk key
                            // so the stored value stays driver-independent.
                            $val = media_path($val);
