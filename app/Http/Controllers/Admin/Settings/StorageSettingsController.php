...
class StorageSettingsController extends Controller
{
    ...
    /**
     * Test a driver's stored (plus any just-typed) credentials WITHOUT persisting them.
     *
     * Persisting first was dangerous: testing the currently-active driver with a typo'd
     * endpoint/bucket immediately rebound the live `public` disk to a broken config and
     * took media serving down site-wide — before the admin ever pressed Save. The probe
     * now runs against an in-memory config; only Save (update()) writes credentials.
     */
    public function test(Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'credentials.bucket' => ['nullable', 'string', 'max:255'],
            'credentials.endpoint' => ['nullable', 'string', 'max:255', 'regex:/^https?:\/\//i'],
            'credentials.url' => ['nullable', 'string', 'max:255', 'regex:/^https?:\/\//i'],
        ]);

        $driver = $validated['driver'];
        $submitted = $validated['credentials'] ?? [];

        // Mirror persistCredentials() semantics so the probe tests exactly what a Save would
        // store: a blank SECRET means "keep the stored one" (the UI never echoes a saved
        // secret back), while an OPTION the UI sends is taken verbatim — including a
        // deliberately cleared one.
        $stored = fn (string $field) => (string) (settings("storage_{$driver}_{$field}") ?? '');
        $secret = fn (string $field) => filled($submitted[$field] ?? null)
            ? (string) $submitted[$field]
            : $stored($field);
        $option = fn (string $field) => array_key_exists($field, $submitted)
            ? (string) ($submitted[$field] ?? '')
            : $stored($field);

        $probe = new CloudStorageService(
            driver: $driver,
        ...
        return response()->json($probe->testConnection());
    }
    ...
    {
        // `settings.storage` grants this page on its own; `settings.manage` (the broad
        // "all settings" grant) continues to work for roles that already hold it.
        abort_unless(
            auth('admin')->user()?->hasAnyPermission(['settings.storage', 'settings.manage']),
            403
        );
    }
}
