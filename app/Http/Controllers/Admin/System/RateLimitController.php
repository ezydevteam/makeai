...
class RateLimitController extends Controller
{
    ...
    public function updateTiers(Request $request): RedirectResponse
    {
        ...
        foreach ($validated['tiers'] as $tier) {
            ...
            foreach ([
                'guest' => ['max' => $tier['guest_max'], 'window' => $tier['guest_window']],
                'free_user' => ['max' => $tier['free_max'], 'window' => $tier['free_window']],
                'pro_user' => ['max' => $tier['pro_max'], 'window' => $tier['pro_window']],
            ] as $tierName => $vals) {
                RateLimitRule::updateOrCreate(
                    ['category' => $category, 'tier' => $tierName],
                    ['max_attempts' => $vals['max'], 'window_seconds' => $vals['window']],
                );
            }
        }
    ...
    private function buildTierConfig(): array
    {
        ...
        foreach (array_keys(self::CATEGORIES) as $category) {
            ...
            foreach (self::TIERS as $tier) {
                ...
                $rule = $rules[$category][$tier] ?? null;

                $maxVal = (int) ($rule['max_attempts'] ?? 0);
                $windowVal = (int) ($rule['window_seconds'] ?? 0);
