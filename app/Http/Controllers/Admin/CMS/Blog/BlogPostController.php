...
class BlogPostController extends Controller
{
    ...
    private function storeUploadedFeaturedImage(BlogPostRequest $request, array $validated): array
    {
        ...
        // Persist the RELATIVE disk key, never a URL. Storage::url() resolved against the
        // default disk (not `public`), so it baked a "/storage/..." path into the row that
        // 404s the moment media moves to a cloud bucket. Readers call media_url()/mediaUrl().
        $validated['featured_image'] = store_public_upload($request->file('featured_image_file'), 'blog');

        return $validated;
    }

    /**
     * Remove a replaced featured image from the media disk.
     *
     * This method was CALLED from update() but never defined, so replacing a post's
     * featured image raised "Call to undefined method" (a hard 500). media_path()
     * normalises whichever format the row holds — a relative key, a legacy
     * "/storage/..." URL, or a full bucket/CDN URL — down to the disk key. An external
     * URL that isn't ours reduces to something not on the disk, so nothing is deleted.
     */
    private function deleteStoredImage(?string $image): void
    {
        $key = media_path($image);

        if ($key === '' || preg_match('#^(https?:)?//#i', $key)) {
            return;
        }
