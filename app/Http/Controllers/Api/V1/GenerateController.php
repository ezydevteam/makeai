...
class GenerateController extends Controller
{
    ...
    public function stream(Request $request): StreamedResponse
    {
        ...
        // Build prompt
        $fields = $this->applyRequestedModel($validated['fields'], $validated['model'] ?? null);
        ...
        if ($isRefine) {
            ...
            $completion = $this->promptBuilder->buildRefine($template, $refineContent, $refineInstruction, $fields['model'] ?? null, $user);
        } else {
    ...
    public function text(Request $request): JsonResponse
    {
        ...
        // Integration-backed tools (grammar / AI-detector / plagiarism / translation)
        // must run their engine here too — stream() branches on this, and without the
        // same branch the sync endpoint quietly sent them to the LLM and billed per
        // token instead of the integration's fixed per-call credit cost.
        if ($template->usesIntegration()) {
            return $this->textUtility($template, $fields, $user, $idempotencyKey);
        }
    ...
    /**
     * Put the requested model into the fields the PromptBuilder sees — but only if
     * the admin has activated it.
     *
     * The frontend sends `model: ''` for a tool with no model picker, so this must
     * test for a non-empty string rather than a non-null one. It also drops any
     * `model` smuggled directly into the fields array, which is not validated.
     */
    private function applyRequestedModel(array $fields, ?string $requested): array
    {
        unset($fields['model']);
        ...
        if ($requested !== '') {
            $this->assertModelAllowed($requested);
            $fields['model'] = $requested;
        }
    ...
    /**
     * Sync counterpart of streamUtility() — run the integration engine and return
     * its formatted result in the same JSON envelope as an LLM generation.
     */
    private function textUtility(AiTool $template, array $fields, ?User $user, ?string $idempotencyKey): JsonResponse
    {
        ...
        if (($result['ok'] ?? false) !== true) {
            return response()->json([
                'success' => false,
                'message' => $result['error'] ?? translate('The tool could not complete. Please try again.'),
            ], 422);
        }
