...
class AiController extends Controller
{
    ...
    public function runTemplate(Request $request, AiTool $template): JsonResponse
    {
        ...
        // Enforce the tool's access level (pro / plan-required). The web /generate
        // path already does this; this API route must too, or plan gating is bypassed.
        $access = app(ToolAccessService::class)->checkAccess($template, $request->user());
        if (! $access->allowed) {
            return response()->json(
                ['success' => false, 'message' => translate('You do not have access to this tool.')],
