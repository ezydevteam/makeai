...
class PricingController extends Controller
{
    public function index(PlanPriceResolver $resolver)
    {
        ...
        $pageTitle = settings('pricing_meta_title') ?: translate('Pricing');
        $description = settings('pricing_meta_description')
            ?: translate('Compare :app plans and pricing. Pick the plan that fits your usage and upgrade or cancel any time.', ['app' => $siteName]);

        return Inertia::render('Pricing', [
            'plans' => $resolver->resolveCollection($plans, $countryCode)->values(),
            'pricingCountry' => $countryCode,
            'currentSubscription' => $this->currentSubscription(),
            // Public, indexable page — it previously shipped no seo payload at all, so
            // app.blade.php fell back to the bare site name and emitted no canonical.
            // title_page stays site-free: Pricing.vue passes it to <Head :title>, which
            // the global callback suffixes with the site name and admin's separator.
            'seo' => [
                'title' => document_title($pageTitle),
                'title_page' => $pageTitle,
                'description' => $description,
                // Query-less, so ?country= variants consolidate to the one pricing URL.
