...
class PrivacyController extends Controller
{
    public function index(Request $request)
    {
        ...
        return Inertia::render('User/Privacy', [
            ...
            // The SMS preference is only meaningful once the user is actually
            // reachable by text (verified phone + configured gateway).
            'smsAvailable' => user_can_receive_sms($user),
            'optinPreferencesEnabled' => (bool) settings('optin_preferences_enabled', true),
            'cookiePreferencesEnabled' => (bool) settings('cookie_preferences_enabled', true),
        ]);
    }
    ...
    public function updatePreferences(Request $request): RedirectResponse
    {
        ...
        // Consent can only be granted by someone we can actually text; opting OUT is
        // always honoured, whatever the gateway/verification state.
        if (($validated['sms_marketing_opt_in'] ?? false) && ! user_can_receive_sms($user)) {
            $validated['sms_marketing_opt_in'] = false;
        }

        // Admin-disabled sections are not just hidden — their values are ignored, so a
        // stale form or crafted request cannot write preferences the site doesn't offer.
        if (! (bool) settings('optin_preferences_enabled', true)) {
            unset($validated['email_marketing'], $validated['allow_data_improve'], $validated['sms_marketing_opt_in']);
        }

        if (! (bool) settings('cookie_preferences_enabled', true)) {
            unset($validated['cookie_consent']);
        }
