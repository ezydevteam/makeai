...
class PlaygroundController extends Controller
{
    ...
    public function run(Request $request): StreamedResponse
    {
        ...
        // The picker only offers models the admin enabled, but this endpoint is a
        // plain POST: without this check a user can name any provider (500 out of
        // ProviderRegistry) or any model — and a model with no AiModel row bills at
        // TokenGuard's generic fallback rate instead of its real one.
        $this->assertModelEnabled($validated['provider'], $validated['model']);
        ...
        return response()->stream(function () use ($provider, $modelName, $validated, $temperature, $maxTokens, $topP, $user, $providerName) {
            ...
            try {
                ...
                if (! connection_aborted()) {
                    ...
                    // Charge the aborted stream: the provider generated (and billed us
                    // for) these tokens either way. Passing deductCredits=false here made
                    // a cancelled run free in both quota and metered mode — GenerateController
                    // bills the same case. $success=false still logs it as 'cancelled'.
                    TokenGuard::after(
                        $user, $inputTokens, $outputTokens,
                        $modelName, $providerName, 'playground',
    ...
    /**
     * A playground run may only use a provider/model pair the admin has enabled —
     * the same set index() offers in the picker.
     */
    private function assertModelEnabled(string $provider, string $model): void
    {
        $enabled = ProviderRegistry::getEnabledProviders();

        if (! isset($enabled[$provider]) || ! in_array($model, array_values($enabled[$provider]), true)) {
            throw ValidationException::withMessages([
                'model' => translate('That model is not available. Please pick one from the list.'),
            ]);
        }
    }
}
