...
class BlogController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        return Inertia::render('Blog/Index', [
            ...
            'seo' => $this->listingSeo(translate('Blog'), translate('Latest articles, guides, and product updates.')),
    ...
    public function category(Request $request, string $slug): Response
    {
        ...
        return Inertia::render('Blog/Index', [
            ...
            'seo' => $this->listingSeo($category->meta_title ?: $category->name, $category->meta_description ?: $category->description),
            'blogSettings' => $this->blogSettings(),
        ]);
    }
    ...
    public function tag(Request $request, string $slug): Response
    {
        ...
        return Inertia::render('Blog/Index', [
            ...
            'seo' => $this->listingSeo($tag->meta_title ?: $tag->name, $tag->meta_description ?: $tag->description),
            'blogSettings' => $this->blogSettings(),
        ]);
    }
    ...
    public function show(Request $request, string $slug): Response
    {
        ...
        return Inertia::render('Blog/Show', [
            ...
            'share' => $blogSettings['post_social_share_position'] !== 'hide'
    ...
    /**
     * Server-rendered SEO payload (read by app.blade.php) for a single post: maps
     * the flat postMeta()/schema() into the nested og/twitter + schemas contract so
     * crawlers see article title/description/canonical/OG/JSON-LD without running JS.
     */
    private function postSeo(BlogPost $post): array
    {
        $meta = $this->postMeta($post);

        // postMeta() already absolutizes og_image via url(); keep as-is.
        $image = $meta['og_image'] ?: null;

        return [
            // Full document title (with site) → server-rendered <title inertia>.
            'title' => $meta['document_title'],
            'description' => $meta['description'],
            'keywords' => $meta['keywords'] ?: null,
            'canonical' => $meta['canonical'],
            'robots' => $post->no_index ? 'noindex, follow' : null,
            'og' => [
                'title' => $meta['og_title'],
                ...
                'type' => 'article',
                'url' => $meta['canonical'],
                // article:* timestamps — app.blade.php emits these for og:type=article
                // only. The JSON-LD already carries the same dates; these are the
                // OpenGraph equivalent that social scrapers read.
                'published_time' => $post->published_at?->toAtomString(),
                'modified_time' => Carbon::parse($post->updated_at)->toAtomString(),
            ],
            'twitter' => [
    ...
    private function postMeta(BlogPost $post): array
    {
        ...
        return [
            ...
            // browser tab are byte-identical — document_title() applies the admin's
            // separator, the same one app.ts appends client-side. `title` above stays
            // site-free for OG/Twitter reuse.
    ...
    private function sidebarCategories()
    {
        return BlogCategory::active()
            ->where('posts_count', '>', 0)
            ->orderBy('sort_order')
            ->get(['id', 'name', 'slug', 'icon', 'color', 'posts_count']);
    }
    ...
    /**
     * Server-rendered SEO payload for a blog listing/archive page (read by
     * app.blade.php). Canonical is the query-less path (url()->current() drops
     * ?page/?search/?sort), so paginated/filtered/sorted variants consolidate to
     * the one archive URL instead of spawning near-duplicate indexable pages.
     */
    private function listingSeo(string $title, ?string $description): array
    {
        $description = $description ?: translate('Latest articles, guides, and product updates.');

        return [
            // Full document title: Blog/Index.vue passes the site-free `meta.title` to
            // <Head :title>, which the global callback suffixes. Emitting the bare title
            // here left the server tag ("Blog") disagreeing with the tab ("Blog | Site").
    ...
    {
        return [
            'sidebar_position' => settings('blog_sidebar_position', 'right'),
