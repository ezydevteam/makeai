...
class SubscriptionController extends Controller
{
    ...
    public function downgrade(
        ...
        // A downgrade is a *scheduled, unbilled* plan switch — it can only target a
        // cycle the gateway is able to auto-bill at the next renewal. Lifetime is a
        // one-time purchase with no recurring price and no period end, so scheduling
        // one would hand the user a permanent plan for free. It must go through
        // checkout like any other purchase.
        if ($data['billing'] === 'lifetime') {
            return back()->with('error', translate('Lifetime is a one-time purchase — buy it from the pricing page instead of scheduling a downgrade.'));
        }
        ...
        // Only a gateway that can swap the recurring price deterministically can auto-bill
        // a lower paid plan. Everything else — a free target, a one-time gateway, or a
        // gateway whose swap needs buyer approval (PayPal) or simply failed — ends at
        // period end → Free.
        //
        // The gateway swap is attempted BEFORE anything is scheduled, and the schedule is
        // only recorded if the gateway accepted it. Scheduling first (as this did) meant a
        // rejected or unapproved swap still flipped the local plan at period end while the
        // gateway carried on billing the old price.
        $currentPlanName = $subscription->plan->name;
        $isCycleChange = $target->id === $subscription->plan_id;
        $date = $subscription->current_period_end?->toFormattedDateString();

        $inPlace = ! $target->is_free
            && $modifier->supportsInPlaceDowngrade($subscription)
