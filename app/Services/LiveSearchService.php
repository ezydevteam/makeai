...
class LiveSearchService
{
    ...
    public function searchPublic(string $term): array
    {
        ...
        if ($sources['categories']) {
            $groups[] = $this->group('ai_categories', translate('AI Categories'), $this->searchAiCategories($term));
            $groups[] = $this->group('blog_categories', translate('Blog Categories'), $this->searchBlogCategories($term));
        }
    ...
    private function searchAiCategories(string $term): Collection
    {
        return Category::query()
            ...
            ->limit(5)
            ->get(['id', 'name', 'slug', 'description', 'tools_count'])
            ->map(fn (Category $category): array => [
                'key' => 'ai-category-'.$category->slug,
                'title' => $category->name,
                'excerpt' => $this->highlight($category->description ?: translate('AI tool category'), $term),
                'url' => route('ai.tools.index', ['category' => $category->slug]),
                'meta' => translate('AI Category'),
            ]);
    }

    private function searchBlogCategories(string $term): Collection
    {
        return BlogCategory::query()
            ...
            ->limit(5)
            ->get(['id', 'name', 'slug', 'description', 'posts_count'])
            ->map(fn (BlogCategory $category): array => [
                'key' => 'blog-category-'.$category->slug,
                'title' => $category->name,
                'excerpt' => $this->highlight($category->description ?: translate('Blog category'), $term),
                'url' => route('blog.category', $category->slug),
                'meta' => translate('Blog Category'),
            ]);
    }
    ...
    private function searchWithFullTextFallback(Builder $query, array $columns, string $term, callable $fallbackOrder): Collection
    {
        if ($this->canUseFullText()) {
            try {
                ...
                // Natural-language mode ignores short (< ft_min_word_len) and partial
                // terms, returning nothing without error. Fall through to LIKE so those
                // queries still match instead of silently yielding no results.
    ...
    private function adminMenuItems(): array
    {
        return [
            ...
            ['title' => 'Integrations', 'section' => 'Settings', 'route' => 'admin.settings.integrations.index', 'permissions' => ['ai.tools'], 'description' => 'Configure third-party service integrations.', 'keywords' => 'pexels unsplash search transcript api'],
            ...
            ['title' => 'System', 'section' => 'System', 'route' => 'admin.system.health', 'permissions' => ['settings.manage'], 'description' => 'Health check, updates, cron jobs, maintenance mode, and admin appearance.', 'keywords' => 'system health update cron maintenance appearance'],
