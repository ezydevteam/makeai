...
class GatewaySubscriptionModifier
{
    ...
    /**
     * Move the subscription to the target plan, effective at the next renewal
     * (no mid-cycle charge). Used for scheduled downgrades.
     */
    public function swapAtPeriodEnd(GatewaySubscription $subscription, Plan $target, string $billingCycle): void
    {
        if ($subscription->gateway === 'paypal') {
            $this->revisePayPal($subscription, $this->paypalPlanId($target, $billingCycle));

            return;
        }

        $this->swapStripe($subscription, $this->stripePriceId($target, $billingCycle));
    }
    ...
    private function swapStripe(GatewaySubscription $subscription, ?string $newPriceId): void
    {
        if ($subscription->gateway !== 'stripe' || ! $subscription->gateway_subscription_id || ! config('cashier.secret')) {
            return;
        }
