...
class AddonLicenseService
{
    ...
    public function checkAddonUpdate(string $addonSlug): array
    {
        ...
        // flow are testable locally. Control via the addon 'test_latest_version' setting.
        if (PurchaseCode::testModeActive()) {
            $latest = (string) addon_setting($addonSlug, 'test_latest_version', $this->bumpVersion($current));
