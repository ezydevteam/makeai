...
final class ProductDocsService
{
    ...
    {
        return $this->path ?? resource_path('docs/product');
    }
    ...
    private function fingerprint(): string
    {
        ...
        $parts = array_map(
            fn (string $file): string => basename($file) . '|' . (@filemtime($file) ?: 0) . '|' . (@filesize($file) ?: 0),
            $files
        );
    ...
    {
        $found = glob(rtrim($this->path(), '/\\') . DIRECTORY_SEPARATOR . '*.md');
