...
class ProviderRegistry
{
    ...
    public static function resolve(string $name): AiDriverInterface
    {
        ...
        return self::makeDriver($driverName, $apiKey);
    }

    /**
     * Drivers are built through the container-bound factory, not `new`, so a test or
     * an addon can supply its own driver without reaching into every call site.
     */
    private static function makeDriver(string $driverName, ?string $apiKey = null): AiDriverInterface
    {
        return app(AiDriverFactory::class)->make($driverName, $apiKey);
    }
    ...
    public static function configured(): array
    {
        ...
        foreach (self::$providers as $name => $driverName) {
            try {
                $driver = self::makeDriver($driverName);
