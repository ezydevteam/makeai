...
class AiService
{
     ...
     *
     * Pass $user to bill the embedding to them. Embedding a large document is a real
     * provider cost, and these calls used to pass a null user unconditionally — so the
     * tokens were counted toward the global budget but charged to nobody, and the
     * usage row (which TokenGuard only writes when there is a user) was never created.
     */
    public function embedText(
        ...
        try {
            ...
            TokenGuard::recordFailure($user, $providerName, $model ?? 'unknown', 'embedding', 0, 0, [
                'error' => $e->getMessage(),
                'tool_slug' => 'embedding',
            ]);
            throw $e;
     ...
     *
     * Pass $user to bill the batch to them — see embedText().
     *
     * @param  string[]  $texts
    ...
    public function embedBatch(
        ...
        try {
            TokenGuard::before($user, null, null);
            ...
            TokenGuard::recordFailure($user, $providerName, 'unknown', 'embedding', 0, 0, [
                'error' => $e->getMessage(),
