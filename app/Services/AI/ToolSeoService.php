...
class ToolSeoService
{
    ...
    {
        // Two title forms, because the server head and the client head need different
        // halves of the same string:
        //   - title_page: site-free, e.g. "AI Writer - Free AI Tool". ToolPage.vue puts
        //     THIS in its <Head> <title>; Inertia's title callback appends the site name.
        //     Passing the full title there yields "… | MakeAI | MakeAI".
        //   - title: the complete tag for app.blade.php's <title inertia>, which no
        //     callback touches. document_title() composes it, so the admin's separator
        //     choice reaches this half and the client half identically.
        // Any hyphen lives inside the meta_title itself. No invented "Free AI Tool"
        // fallback — a tool without a meta_title just uses its name.
        $titlePage = filled($tool->meta_title) ? $tool->meta_title : $tool->name;
        $title = document_title($titlePage);
        $description = $tool->meta_description
                            ?? $this->clampText(translate('Use :tool for free. :description', [
                                'tool' => $tool->name,
    ...
    {
        $appName = settings('app_name', config('app.name', 'MakeAI'));
    ...
    /**
     * Word-safe truncate to a max character length (no mid-word cut, no trailing
     * ellipsis) for auto-generated meta title/description.
     */
    private function clampText(string $text, int $max): string
    {
    ...
    private function generateKeywords(AiTool $tool): string
    {
        ...
        $keywords = [
            ...
            settings('app_name', config('app.name', 'MakeAI')),
        ];
