...
Route::middleware(['admin.auth', 'admin.audit'])->group(function () {
    ...
    // Rate Limits
    Route::prefix('system/rate-limits')->name('admin.system.rate-limits.')->middleware('admin.permission:system.rate_limits,settings.manage')->group(function () {
    ...
    // Permissions are aligned with the sidebar links so an admin never sees a link
    // they cannot use (or vice-versa): read pages use the "view" permission the
    // sidebar checks; money-mutations require the payments.gateways ("manage")
    // permission. Super-admins bypass all checks.
    // Defense-in-depth: the money-mutation actions (coupon/plan/gateway/subscription/
    // transaction/credit-settings writes) ALSO re-assert the same permission inside
    // the controller via AuthorizesAdminActions::authorizeAdmin(), so a route ever
    // moved out of its permission group still fails closed. Keep the route slug and
    // the in-controller slug in sync when changing either.
    ...
    Route::middleware('admin.permission:marketing.newsletter,users.manage')->group(function () {
        // Bulk SMS (transactional/opted-in recipients only)
        Route::get('marketing/bulk-sms', [\App\Http\Controllers\Admin\Marketing\SmsCampaignController::class, 'index'])->name('admin.bulk-sms.index');
        Route::post('marketing/bulk-sms', [\App\Http\Controllers\Admin\Marketing\SmsCampaignController::class, 'store'])->name('admin.bulk-sms.store');
        Route::get('marketing/bulk-sms/{campaign}', [\App\Http\Controllers\Admin\Marketing\SmsCampaignController::class, 'show'])->name('admin.bulk-sms.show');
        Route::post('marketing/bulk-sms/{campaign}/retry', [\App\Http\Controllers\Admin\Marketing\SmsCampaignController::class, 'retry'])->name('admin.bulk-sms.retry');
    ...
    Route::middleware(['contact', 'admin.permission:contact.settings,content.pages'])->group(function () {
    ...
    // Ads System
    Route::middleware('admin.permission:marketing.ads,settings.manage')->group(function () {
    ...
    // Social Counters
    Route::middleware('admin.permission:settings.social,settings.manage')->group(function () {
    ...
    // OAuth
    Route::middleware('admin.permission:settings.oauth,settings.manage')->group(function () {
    ...
    // Integrations
    Route::middleware('admin.permission:settings.integrations,settings.manage')->group(function () {
    ...
    // Extensions
    Route::middleware('admin.permission:settings.extensions,settings.manage')->group(function () {
    ...
    // GDPR Settings
    Route::middleware('admin.permission:settings.gdpr,settings.manage')->group(function () {
    ...
    Route::middleware('admin.permission:settings.storage,settings.manage')->group(function () {
    ...
    Route::middleware('admin.permission:settings.license')->group(function () {
        Route::get('system/license', [LicenseSettingsController::class, 'edit'])->name('admin.license.settings');
        Route::post('system/license/activate', [LicenseSettingsController::class, 'activate'])->name('admin.license.activate');
        Route::post('system/license/deactivate', [LicenseSettingsController::class, 'deactivate'])->name('admin.license.deactivate');
        Route::post('system/license/reverify', [LicenseSettingsController::class, 'reverify'])->name('admin.license.reverify');
    });
    ...
    Route::middleware('admin.permission:system.cache,settings.manage')->group(function () {
        Route::post('system/cache', [SystemController::class, 'clearCache'])->name('admin.system.cache.clear');
    });

    Route::middleware('admin.permission:system.updates,settings.manage')->group(function () {
    ...
    Route::middleware('admin.permission:system.maintenance,settings.manage')->group(function () {
    ...
    Route::middleware('admin.permission:settings.notifications,settings.general,settings.manage')->group(function () {
    ...
    Route::middleware('admin.permission:marketing.announcements,content.pages')->group(function () {
    ...
    Route::prefix('ai')->name('admin.ai.')->middleware('admin.permission:ai.tools')->group(function () {
        ...
        // NOTE: Tool Reviews are deliberately NOT in this group — they are gated on their own
        // `ai.reviews` permission below. A nested middleware group would only ADD to this
        // group's `ai.tools` requirement, so a reviews-only role could never get in.
    ...
    // Tool Reviews. Own permission (`ai.reviews`) so review moderation can be delegated
    // without handing over the whole AI Tools section; `ai.tools` still opens it, so roles
