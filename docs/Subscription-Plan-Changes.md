...
- **Recurring (Stripe or PayPal), to a lower paid plan:** at the next renewal the
  account is billed the **lower** plan's price and switches to it automatically.
- **One-time gateways, or any downgrade to Free:** the current plan simply runs to
  the end of the period, then the account moves to **Free** (a one-time gateway
  can't auto-charge a lower plan, so the user purchases it again whenever they
  want). The user sees a clear message and can cancel a scheduled change any time
  before it applies.
...
- **Lifetime** plans are always one-time (no recurring PayPal plan).
