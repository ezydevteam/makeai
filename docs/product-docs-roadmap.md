...
**Date:** 2026-07-12
**Scope:** `resources/docs/core/` + `addons/<slug>/docs/` (bundled corpus) + `https://makeai-docs.ezydev.net` (public site)
...
The five pages currently in `resources/docs/core/` are **pipeline fixtures, not the
...
Write each page once as markdown in the corpus (`resources/docs/core/`, or the addon's `docs/`), and build the static site from
...
Every page is a markdown file in the corpus with this front-matter:
...
4. Storage and file uploads *(fixture exists)*
