...
- **Downgrade on Stripe** (lower plan, or yearly→monthly): **scheduled for period
  end** — no charge, no refund; current features stay active until then, then it
  switches and Stripe bills the lower price.
- **Downgrade to Free, on PayPal, or on a one-time gateway**: ends at period end →
  Free, and the lower plan can be purchased any time. (A PayPal plan change can
  need the buyer's approval, so we never promise a lower price PayPal might not
  bill.)
- **Lifetime** is a **purchase**, not a downgrade — buy it from the pricing page.
...
No charge, no refund — they keep their current features until the end of the paid
period either way. On **Stripe** the switch is **scheduled** and the lower price is
billed at the next renewal. On **PayPal**, a one-time gateway, or any downgrade to
**Free**, the plan simply ends at period end and the account moves to Free (the
lower plan can be bought again any time).
